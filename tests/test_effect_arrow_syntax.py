...
""",
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            subprocess.run(
                [str(ROOT / "monad"), str(source), "-o", str(work / "valid")],
...
""")
            ...
            for source in (provider, consumer):
                subprocess.run(
                    [str(ROOT / "monad"), str(source), "-o", str(work / source.stem)],
                    cwd=work, env=env, check=True,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True,
                )
...
""")
            ...
            env["MONAD_CORE"] = str(ROOT / "core")
            subprocess.run(
                [str(ROOT / "monad"), str(bootstrap), "--emit-obj",
...
""")
            subprocess.run(
                [str(ROOT / "monad"), str(consumer), "--emit-obj",
                 "-o", str(work / "consumer")],
                cwd=work, env=env, check=True,
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True,
            )
