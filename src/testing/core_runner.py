...
def print_result(result: ModuleResult) -> None:
    ...
    duration = format_duration(result.elapsed_ns).strip()
    checks = f"{result.assertions} assertion{'s' if result.assertions != 1 else ''}"
    marker_kind = "ok" if result.passed else "fail"
    marker_color = UI.theme.success if result.passed else UI.theme.error
    prefix = f"{UI.paint(marker_color, UI.mark(marker_kind))} "
    UI.emit(prefix, f"{name}  {UI.dim(duration)}  {UI.dim(checks)}")
...
def main() -> int:
    ...
    UI.header("core-test", "core + prelude module verification")
    UI.section("Selection")
    UI.row("modules", str(len(modules)))
    ...
    UI.section("Summary")
    UI.row("modules", str(len(results)))
    UI.row("assertions", str(assertion_total))
    UI.row("passed", str(passed), UI.paint(UI.theme.success, UI.mark("ok")))
    failed_mark = UI.paint(UI.theme.error, UI.mark("fail")) if failures else UI.dim("-")
    UI.row("failed", str(len(failures)), failed_mark)
    UI.row("time", format_duration(elapsed_ns).strip())

    if failures:
        UI.fail(f"{len(failures)} core/prelude module{'s' if len(failures) != 1 else ''} failed")
        return 1
    UI.ok("all core/prelude modules passed")
    return 0
