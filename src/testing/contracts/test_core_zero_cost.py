...
from src.testing.monad_binary import generated_executable, resolve_monad_binary
...
"""
    ...
    def test_core_ordering_is_a_compact_finite_set(self):
        source = """\
(module Main)
(import Data.Ord)
(define (keep-order [x : Ordering] -> Ordering) x)
(show (keep-order GT))
"""
        with tempfile.TemporaryDirectory(prefix="monadc-ordering-zero-cost-") as td:
            temp = Path(td)
            home = temp / "home"
            src = temp / "ordering.mon"
            out = temp / "ordering"
            home.mkdir()
            src.write_text(source, encoding="utf-8")
            ...
            self.assertEqual(execution.returncode, 0, execution.stdout)
            self.assertEqual(execution.stdout.strip(), "GT")

            ir = src.with_suffix(".ll").read_text(encoding="utf-8")
            self.assertRegex(ir, r'define\s+i2\s+@(?:"keep-order"|keep-order)\(i2')
