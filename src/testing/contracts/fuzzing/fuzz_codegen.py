...
* output formatted like src/testing/runner.py so Emacs compilation-mode can buttonize it
...
FUZZ_ROOT = ROOT / "src" / "testing" / "data" / "fuzzing"
PROPERTY_ROOT = FUZZ_ROOT / "properties"
STATE_ROOT = ROOT / "build" / "test" / "fuzzing"
MONAD = resolve_monad_binary()
LAST_ROOT = STATE_ROOT / "last"
RESULTS_FILE = STATE_ROOT / "results.json"
HISTORY_FILE = STATE_ROOT / "history.json"
FIRST_FAILURE_FILE = STATE_ROOT / "first-failure.org"
WIDTH = 118
...
### Output copied in spirit from src/testing/runner.py
...
def load_property(path: Path, *, disabled: bool = False) -> PropertySpec:
    ...
    imports = split_csv(data.get("imports", ""))
    law_owner = data.get("law-owner")
    law_name = data.get("law-name")
    instance = data.get("instance")
    core_law_fields = (law_owner, law_name, instance)
    if any(core_law_fields) and not all(core_law_fields):
        raise ValueError(
            f"{path}: law-owner, law-name, and instance must be declared together"
        )
...
def module_header(seed: int, imports: Sequence[str] = ()) -> list[str]:
    return [
        ";; generated by src/testing/contracts/fuzzing/fuzz_codegen.py",
        f";; seed: {seed}",
...
def build_program(seed: int, cases: Sequence[GeneratedCase]) -> tuple[str, str]:
    imports = tuple(sorted({module for case in cases for module in case.spec.imports}))
    lines = module_header(seed, imports)
    expected: list[str] = []
    ...
    if outcome is None:
        FIRST_FAILURE_FILE.write_text(
            "\n".join([
                ...
                f"#+DATE: {timestamp}",
                "",
                "[OBS id:obs.fuzz.last-first-failure src:src/testing/contracts/fuzzing/fuzz_codegen.py conf:high]",
