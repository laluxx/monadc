...
'''
        with tempfile.TemporaryDirectory() as directory:
            ...
            compiled = subprocess.run(
                [str(resolve_monad_binary()), str(source), "-o", str(executable)],
...
'''
        with tempfile.TemporaryDirectory() as directory:
            ...
            environment["MONAD_CORE"] = str(ROOT / "core")
            compiled = subprocess.run(
                [str(resolve_monad_binary()), str(source)], cwd=ROOT,
...
''')
            ...
            seeded = subprocess.run(
                [str(resolve_monad_binary()), str(consumer), "--emit-obj"], cwd=work,
                env=environment, text=True,
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            ...
            compiled = subprocess.run(
                [str(resolve_monad_binary()), str(forged_consumer)], cwd=work,
                env=environment, text=True,
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            self.assertNotEqual(compiled.returncode, 0, compiled.stdout)
            self.assertIn(
