...
SHADER = ROOT / "src" / "testing" / "contracts" / "fixtures" / "spirv_include.vert"
...
def test_spirv_command_emits_a_valid_monad_module(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [str(resolve_monad_binary()), str(output), "-o", str(tmp_path / "shader-module")],
        cwd=ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_spirv_command_requires_a_monad_output(tmp_path: Path) -> None:
    result = subprocess.run(
        [str(resolve_monad_binary()), "spirv", str(SHADER), "-o", str(tmp_path / "bad.c")],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
...
def test_spirv_command_derives_binding_name_from_shader(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [str(resolve_monad_binary()), "spirv", str(SHADER), "-o", str(output)],
        cwd=ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_spirv_command_derives_output_module_when_omitted(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [str(resolve_monad_binary()), "spirv", "blood.frag"],
        cwd=tmp_path,
        check=True,
        capture_output=True,
        text=True,
...
def test_spirv_command_fragment_and_vertex_outputs_never_collide(
    ...
    subprocess.run(
        [str(resolve_monad_binary()), "spirv", "Triangle.frag"],
        cwd=tmp_path,
        check=True,
        capture_output=True,
        text=True,
    ...
    subprocess.run(
        [str(resolve_monad_binary()), "spirv", "Triangle.vert"],
        cwd=tmp_path,
        check=True,
        capture_output=True,
        text=True,
