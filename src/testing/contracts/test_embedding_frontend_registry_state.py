"""Reader, type, and feature registries are compiler-state owned."""
...
FIXTURE = ROOT / "src/testing/contracts/fixtures/embedding/branches/frontend-state/atom.embedding.25.frontend-state.registry-ownership"


class FrontendRegistryStateTests(unittest.TestCase):
    def test_persistent_registries_follow_state(self):
        subprocess.run(["make", "libmonad-compiler.a"], cwd=ROOT, check=True,
                       text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        with tempfile.TemporaryDirectory(prefix="monadc-registry-state-") as directory:
