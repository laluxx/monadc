#!/usr/bin/env python3
"""Tests for the context garden graph exporter (src/tooling/context/context_graph.py)."""
...
def load_graph():
    script = ROOT / "src" / "tooling" / "context" / "context_graph.py"
...
class TestGraphOutput(unittest.TestCase):
    def test_graph_output_is_valid_json(self):
        result = subprocess.run(
            [sys.executable, str(ROOT / "src" / "tooling" / "context" / "context_graph.py"), "--skip-info", "--compact"],
    ...
    def test_graph_stats_only(self):
        result = subprocess.run(
            [sys.executable, str(ROOT / "src" / "tooling" / "context" / "context_graph.py"), "--stats-only"],
            capture_output=True, text=True,
        )
        self.assertEqual(result.returncode, 0)
    ...
    def test_graph_minimal_omits_raw_content(self):
        result = subprocess.run(
            [sys.executable, str(ROOT / "src" / "tooling" / "context" / "context_graph.py"), "--minimal", "--compact"],
            capture_output=True, text=True,
        )
        self.assertEqual(result.returncode, 0)
        data = json.loads(result.stdout)
    ...
    def test_graph_skip_info_fewer_nodes(self):
        full = json.loads(subprocess.run(
            [sys.executable, str(ROOT / "src" / "tooling" / "context" / "context_graph.py"), "--compact"],
            capture_output=True, text=True,
        ).stdout)
    ...
    def test_help_prints_usage(self):
        result = subprocess.run(
            [sys.executable, str(ROOT / "src" / "tooling" / "context" / "context_graph.py"), "--help"],
            capture_output=True, text=True,
        )
        self.assertEqual(result.returncode, 0)
