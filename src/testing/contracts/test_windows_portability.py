...
class WindowsPortabilityTests(unittest.TestCase):
    ...
    def test_prelude_detection_accepts_windows_path_separators(self):
        ...
        self.assertIn("registry_path_tail", main_c)
        self.assertIn("registry_path_equal", main_c)
    ...
    def test_test_infrastructure_forces_utf8_on_every_host(self):
        suites = read("src/testing/suites.py")
        runner = read("src/testing/runner.py")

        self.assertIn('env["PYTHONUTF8"] = "1"', suites)
        self.assertIn('encoding="utf-8"', runner)

    def test_posix_only_headers_are_guarded_for_windows_builds(self):
        compat_h = read("src/compat.h")
        cli_c = read("src/cli.c")
        buildsystem_c = read("src/buildsystem.c")
        completion_c = read("src/completion.c")
        config_c = read("src/config.c")
        codegen_c = read("src/codegen.c")
        dep_c = read("src/dep.c")
        infer_c = read("src/infer.c")
        spirv_c = read("src/spirv.c")
        typeclass_c = read("src/typeclass.c")
        runtime_c = read("src/runtime.c")
        lsp_c = read("src/tooling/lsp.c")
        lsp_repl_c = read("src/tooling/lsp_repl.c")
        main_c = read("src/main.c")
        ...
        self.assertIn('#include "compat.h"', infer_c)
        self.assertIn('#include "compat.h"', spirv_c)
        self.assertIn('#include "compat.h"', typeclass_c)
        self.assertIn("#undef RT_STRING", runtime_c)
        self.assertIn("#define lstat stat", compat_h)
        ...
        macro_c = read("src/macro.c")
        reader_syntax_c = read("src/reader_syntax.c")
        self.assertIn("macro_absolute_path", macro_c)
    ...
    def test_repl_uses_orc_jit_not_mcjit(self):
        repl_c = read("src/tooling/repl.c")
        repl_h = read("src/tooling/repl.h")
        codegen_c = read("src/codegen.c")
        codegen_h = read("src/codegen.h")
        main_c = read("src/main.c")
    ...
    def test_host_test_harnesses_share_windows_binary_resolution(self):
        helper = read("src/testing/monad_binary.py")
        tuple_test = read("src/testing/contracts/test_tuple_commas.py")
        run_core = read("src/testing/core_runner.py")

        self.assertIn("MONAD_BINARY", helper)
        self.assertIn('ROOT / "build" / "bin" / "monad.exe"', helper)
