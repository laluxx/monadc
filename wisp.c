...
static MONAD_THREAD_LOCAL ArityTable *g_active_wisp_arities = NULL;
static MONAD_THREAD_LOCAL bool g_wisp_trace_enabled = false;

static bool range_contains(const char *start, const char *end,
                           const char *needle) {
    if (!start || !end || !needle || end < start) return false;
    size_t needle_len = strlen(needle);
    if (needle_len == 0) return true;
    for (const char *p = start; p + needle_len <= end; p++) {
        if (memcmp(p, needle, needle_len) == 0) return true;
    }
    return false;
}
...
static MONAD_THREAD_LOCAL ArityTable g_ffi_arities;
static MONAD_THREAD_LOCAL bool g_ffi_arities_init = false;

struct WispPersistentState {
    ArityTable arities;
    bool initialized;
};

static MONAD_THREAD_LOCAL WispPersistentState *g_bound_persistent_state;
static MONAD_THREAD_LOCAL ArityTable g_saved_ffi_arities;
static MONAD_THREAD_LOCAL bool g_saved_ffi_arities_init;

WispPersistentState *wisp_persistent_state_create(void) {
    return calloc(1, sizeof(WispPersistentState));
}

void wisp_persistent_state_destroy(WispPersistentState *state) {
    if (!state || state == g_bound_persistent_state) return;
    if (state->initialized) arity_free(&state->arities);
    free(state);
}

bool wisp_persistent_state_enter(WispPersistentState *state) {
    if (!state || g_bound_persistent_state) return false;
    g_saved_ffi_arities = g_ffi_arities;
    g_saved_ffi_arities_init = g_ffi_arities_init;
    g_ffi_arities = state->arities;
    g_ffi_arities_init = state->initialized;
    memset(&state->arities, 0, sizeof(state->arities));
    state->initialized = false;
    g_bound_persistent_state = state;
    return true;
}

void wisp_persistent_state_leave(WispPersistentState *state) {
    if (!state || g_bound_persistent_state != state) return;
    state->arities = g_ffi_arities;
    state->initialized = g_ffi_arities_init;
    g_ffi_arities = g_saved_ffi_arities;
    g_ffi_arities_init = g_saved_ffi_arities_init;
    memset(&g_saved_ffi_arities, 0, sizeof(g_saved_ffi_arities));
    g_saved_ffi_arities_init = false;
    g_bound_persistent_state = NULL;
}
...
WispInputStatus wisp_classify_input(const char *source,
    ...
    for (const char *p = source; *p; p++) {
        ...
        if (*p == ';') {
            ...
            /* Apostrophe is also the reader quote prefix.  Enter character
             * mode only for a complete-looking 'x' or escaped '\\x' token;
             * otherwise delimiters following quote remain visible to the
             * completeness scanner. */
            bool escaped_char = p[1] == '\\' && p[2] && p[3] == '\'';
            bool plain_char = p[1] && p[1] != '\n' && p[2] == '\'';
            if (escaped_char || plain_char)
                in_char = true;
        } else if (*p == '(' || *p == '[' || *p == '{') {
            if (depth >= (int)(sizeof(stack) / sizeof(stack[0])))
    ...
    /* Unlike an open indentation block, a canonical glyph tree visibly
     * closes itself with its final catch-all arrow.  This matters for pasted
     * REPL input: the closing ▶ is the terminator, so an extra blank line is
     * neither necessary nor desirable. */
    bool closed_glyph_tree = false;
    const char *last_line = source;
    for (const char *q = source; *q; q++)
        if (*q == '\n' && q[1]) last_line = q + 1;
    while (*last_line == ' ' || *last_line == '\t') last_line++;
    if (strncmp(last_line, "╰─", strlen("╰─")) == 0) {
        const char *q = last_line + strlen("╰─");
        while (strncmp(q, "─", strlen("─")) == 0)
            q += strlen("─");
        if (strncmp(q, "▶", strlen("▶")) == 0) {
            q += strlen("▶");
            while (*q == ' ' || *q == '\t') q++;
            closed_glyph_tree = *q != '\0';
        }
    }
...
static void arity_prescan(ArityTable *t, const char *source) {
    ...
    while (true) {
        ...
        if (tok.type == TOK_SYMBOL && strcmp(tok.value, "layout") == 0) {
            ...
            if (tok.type == TOK_SYMBOL) {
                ...
                while (true) {
                    ...
                    if (tok.type == TOK_EOF) { free(tok.value); break; }
                    /* A vertical layout owns only its header line and later
                     * lines indented beneath `layout`.  Stop before the next
                     * sibling declaration instead of accumulating fields
                     * from every following layout until a known keyword
                     * happens to appear. */
                    if (tok.line > header_line &&
                        tok.column <= layout_column) {
                        free(tok.value);
                        lex = token_start;
                        break;
                    }
                    ...
                    } else if (tok.type == TOK_SYMBOL && tok.value) {
                        /* Vertical Wisp layouts may omit the brackets:
                         *
                         *   layout Point
                         *     x :: Int
                         *     y :: Int
                         *
                         * The field separator must be on the same physical
                         * line, which prevents a type name on the preceding
                         * field from being mistaken for another field. */
                        Lexer peek = lex;
                        Token sep = lexer_next_token(&peek);
                        if (sep.line == tok.line && sep.type == TOK_SYMBOL &&
                            sep.value && strcmp(sep.value, "::") == 0)
                            field_count++;
                        free(sep.value);
                    }
                    free(tok.value);
        ...
        if (tok.type == TOK_SYMBOL && strcmp(tok.value, "data") == 0) {
            ...
            if (in_ctors) {
                ...
                while (tok.type == TOK_SYMBOL || tok.type == TOK_PIPE ||
                       tok.type == TOK_LPAREN || tok.type == TOK_LBRACKET) {
                        ...
                        while (true) {
                            ...
                            if (tok.type == TOK_SYMBOL &&
                                (strcmp(tok.value, "define") == 0 ||
                                 ...
                                 strcmp(tok.value, "layout") == 0 ||
                                 strcmp(tok.value, "type") == 0 ||
                                 strcmp(tok.value, "module") == 0 ||
                                 strcmp(tok.value, "instance") == 0 ||
                                 strcmp(tok.value, "tests") == 0)) {
                                free(tok.value);
                                lex = token_start;
                                data_done = true;
                                break;
            ...
            while (true) {
                ...
                if (mname) {
                    ...
                    while (true) {
                        ...
                        if (tok.type == TOK_ARROW || tok.type == TOK_EFFECT_ARROW ||
                            (tok.type == TOK_SYMBOL && strcmp(tok.value, "->") == 0)) {
                            arity++;
                            free(tok.value);
                            continue;
        ...
        if (tok.type == TOK_SYMBOL &&
            ...
            if (tok.type == TOK_SYMBOL) {
                ...
                if (tok.type == TOK_SYMBOL && strcmp(tok.value, "::") == 0) {
                    ...
                    if (relation_signature && !found_arrow) {
                        arity_set(t, fname, 2);
                    } else if (found_arrow) {
                ...
                } else if (tok.type == TOK_ARROW || tok.type == TOK_EFFECT_ARROW ||
                           (tok.type == TOK_SYMBOL && strcmp(tok.value, "->") == 0)) {
    ...
    /* A relation annotation describes a binary callable value. Nested `->`
     * is type structure rather than a function signature, so record its
     * surface arity explicitly for expressions such as `show r x y`. */
    for (const char *line = source; line && *line; ) {
        const char *end = strchr(line, '\n');
        if (!end) end = line + strlen(line);
        const char *p = line;
        while (p < end && (*p == ' ' || *p == '\t')) p++;
        if ((size_t)(end - p) > 7 && strncmp(p, "define ", 7) == 0) {
            const char *name = p + 7;
            while (name < end && (*name == ' ' || *name == '\t')) name++;
            const char *name_end = name;
            while (name_end < end && *name_end != ' ' && *name_end != '\t')
                name_end++;
            const char *sig = strstr(name_end, "::");
            const char *open = sig && sig < end ? strchr(sig + 2, '{') : NULL;
            const char *arrow = open && open < end ? strstr(open + 1, "->") : NULL;
            const char *close = open && open < end ? strchr(open + 1, '}') : NULL;
            if (name_end > name && arrow && close && arrow < close && close < end) {
                char *relation_name = strndup(name, (size_t)(name_end - name));
                arity_set(t, relation_name, 2);
                free(relation_name);
            }
        }
        line = *end == '\n' ? end + 1 : end;
...
typedef struct WispRecoveryScope {
    char *owned[3];
    WTokenStream stream;
    ArityTable arities;
    SB builder;
    int builder_active;
    ArityTable *saved_active_arities;
    int (*saved_param_kind)(const char *, int);
    int (*saved_known_function)(const char *);
    const char *saved_filename;
    const char *saved_source;
    const char *saved_original_source;
    ReaderDiagnosticCleanup cleanup;
    bool deferred;
    bool protected_scope;
} WispRecoveryScope;

static void wisp_recovery_scope_release(void *opaque) {
    WispRecoveryScope *scope = opaque;
    if (!scope) return;
    if (scope->builder_active) sb_free(&scope->builder);
    wts_free(&scope->stream);
    arity_free(&scope->arities);
    for (size_t i = 0; i < sizeof(scope->owned) / sizeof(scope->owned[0]); i++) {
        free(scope->owned[i]);
        scope->owned[i] = NULL;
    }
    g_active_wisp_arities = scope->saved_active_arities;
    g_param_kind_is_func = scope->saved_param_kind;
    g_is_known_function = scope->saved_known_function;
    current_filename = scope->saved_filename;
    current_source = scope->saved_source;
    original_source = scope->saved_original_source;
    if (scope->protected_scope) {
        reader_syntax_clear();
        macro_clear();
    }
}

static void wisp_recovery_scope_finish(WispRecoveryScope *scope) {
    if (scope->deferred &&
        !reader_diagnostic_cleanup_cancel_current(&scope->cleanup))
        abort();
    scope->deferred = false;
    wisp_recovery_scope_release(scope);
}
...
static const char *wisp_labeled_arrow_end(
    const char *start, char **label) {
    if (label) *label = NULL;
    if (!start || *start != '-' || start[1] == '>') return NULL;
    ...
    for (const char *p = start + 1; p < end; p++)
        if (!(isalnum((unsigned char)*p) || *p == '_' || *p == '.' ||
              *p == '-'))
            return NULL;
    if (label) *label = strndup(start + 1, (size_t)(end - start - 1));
    return end + 2;
}
...
static const char *wisp_find_top_level_arrow(const char *s) {
    ...
    for (const char *q = s; *q; q++) {
        ...
        if (*q == '(' || *q == '[' || *q == '{') depth++;
                 ...
                 (*(q + 1) == '>' || wisp_labeled_arrow_end(q, NULL)))
            return q;
    }
    return NULL;
}
...
static MONAD_THREAD_LOCAL WispPendingType *g_wisp_pending_types = NULL;
...
static bool wisp_split_arrow_signature(const char *sig, int expected_params,
                                       char **param_types, char **effect_names,
                                       char **ret_type) {
    const char *line_end = get_logical_line_end(sig);
    const char *seg_start = sig;
    /* A class context qualifies the complete function type; it is not part
     * of the first parameter type.  The reader-level typed header currently
     * treats contexts as evidence requirements and deliberately skips them,
     * so begin splitting immediately after a top-level `=>`. */
    {
        int context_depth = 0;
        bool context_in_str = false;
        for (const char *q = sig; q + 1 < line_end; q++) {
            if (context_in_str) {
                if (*q == '\\' && q + 1 < line_end) q++;
                else if (*q == '"') context_in_str = false;
                continue;
            }
            if (*q == '"') { context_in_str = true; continue; }
            if (*q == '(' || *q == '[' || *q == '{') context_depth++;
            else if (*q == ')' || *q == ']' || *q == '}') {
                if (context_depth > 0) context_depth--;
            } else if (context_depth == 0 && q[0] == '=' && q[1] == '>') {
                seg_start = q + 2;
    ...
    int depth = 0;
    bool in_str = false;
    for (const char *q = seg_start; q <= line_end; q++) {
        ...
        const char *labeled_end = !at_end && depth == 0
            ? wisp_labeled_arrow_end(q, &effect_name) : NULL;
        bool ordinary_arrow = !at_end && depth == 0 &&
            *q == '-' && *(q + 1) == '>';
        if (at_end || ordinary_arrow || labeled_end) {
            char *seg = wisp_trim_range_dup(seg_start, q);
            if (seg_count < expected_params) {
                param_types[seg_count] = seg;
                effect_names[seg_count] = effect_name;
                effect_name = NULL;
            }
            ...
            seg_count++;
            if (at_end) break;
            q = labeled_end ? labeled_end - 1 : q + 1;
            seg_start = q + 1;
        }
    }
    if (seg_count != expected_params + 1 || !*ret_type) {
        for (int i = 0; i < expected_params; i++) {
            free(param_types[i]); param_types[i] = NULL;
            free(effect_names[i]); effect_names[i] = NULL;
        }
...
/* Preserve postfix indexing while Wisp rearranges an expression into prefix
 * form.  Without this seam, a guard such as `bytes[0] = 0x6e` becomes
 * `(= (bytes 0) 0x6e)`: that is indistinguishable from a call once the
 * generated guard lambda shadows the annotated parameter.  Lowering through
 * the ordinary typed `nth` contract retains the collection/element relation
 * even inside that lambda; the reader-native `__index` path remains in use
 * outside Wisp's guard elaboration. */
static char *wisp_rewrite_postfix_indices(const char *src) {
    SB out;
    sb_init(&out);
    const char *p = src;
    bool in_string = false;
    ...
    while (*p) {
        ...
        if (*p == '"') { in_string = true; sb_putc(&out, *p++); continue; }
        if (*p == '\'') { in_char = true; sb_putc(&out, *p++); continue; }
        ...
        if ((first >= 'A' && first <= 'Z') ||
            ...
            const char *open = p;
            if (*open == '[') {
                const char *index = open + 1;
                const char *end = index;
                while (*end >= '0' && *end <= '9') end++;
                if (end > index && *end == ']') {
                    sb_puts(&out, "(nth ");
                    for (const char *q = name; q < open; q++) sb_putc(&out, *q);
                    sb_putc(&out, ' ');
                    for (const char *q = index; q < end; q++) sb_putc(&out, *q);
                    sb_putc(&out, ')');
                    p = end + 1;
                    continue;
                }
            ...
            for (const char *q = name; q < p; q++) sb_putc(&out, *q);
            continue;
        }
...
static bool wisp_is_single_token_form(const char *src);
...
static char *wisp_expand_expr_snippet(ArityTable *at, const char *src) {
    ...
    /* An explicit contextual judgment is already a reader-owned expression
     * with its own delimiters (`⊢`, optional `≡`, and `:`).  Running it
     * through Wisp's ordinary Pratt/infix expander can promote `+` across the
     * judgment colon when the judgment is the unparenthesized RHS of a value
     * binding.  Preserve the complete surface form and let the judgment
     * parser apply its grammar. */
    if (quote[0] == '{' && strstr(quote, "⊢") != NULL) {
        return strdup(quote);
    }
    /* Mathematical type-definition sugar: `type T = S`.  Equality here is
     * declaration punctuation, not the value-level equality operator. */
    if (strncmp(quote, "type ", 5) == 0) {
        const char *eq = strchr(quote + 5, '=');
        if (eq) {
            const char *name_begin = quote + 5;
            while (*name_begin == ' ' || *name_begin == '\t') name_begin++;
            ...
            SB declaration;
            sb_init(&declaration);
            sb_puts(&declaration, "(type ");
            for (const char *p = name_begin; p < name_end; p++)
                sb_putc(&declaration, *p);
            sb_putc(&declaration, ' ');
            sb_puts(&declaration, body);
            sb_putc(&declaration, ')');
            return sb_take(&declaration);
        }
    }
    if ((quote[0] == '"' || quote[0] == '\'') &&
        wisp_is_single_token_form(quote))
        return strdup(quote);
    if (strncmp(quote, "asm ", 4) == 0) {
        SB assembly;
        sb_init(&assembly);
        sb_putc(&assembly, '(');
        sb_puts(&assembly, quote);
        sb_putc(&assembly, ')');
        return sb_take(&assembly);
    }
    if (quote[0] == '\'' &&
        ((quote[1] == '(' || quote[1] == '[' || quote[1] == '{') ||
         (quote[1] != '\0' && quote[strlen(quote) - 1] != '\'')))
        return strdup(quote);

    char *indexed_src = wisp_rewrite_postfix_indices(src);
    WTokenStream ts = build_token_stream(indexed_src, at);
    free(indexed_src);
    SB sb;
    sb_init(&sb);
    bool first = true;
...
static char *wisp_expand_guard_without_subjects(ArityTable *at,
                                                const char *src) {
    const char *start = src;
    while (*start == ' ' || *start == '\t') start++;
    const char *end = get_logical_line_end(start);

    if (wisp_find_top_level_condition_equal(start, end))
...
static char *wisp_try_expand_simple_where_lambda(ArityTable *at,
    ...
    while (*p && ok) {
        ...
        if (!arrow) {
            /* Canonical multiline guarded definition:
             *
             *   value
             *     | predicate -> result
             *
             * The subject-only first line declares the parameters; the
             * following continuation guards provide the clauses. */
            if (param_count < 0) {
                char *line_params[WISP_MAX_PARAMS] = {0};
                int line_param_count =
                    wisp_parse_simple_names_any(line, line_params,
                                                WISP_MAX_PARAMS);
        ...
        if (!pipe) {
            ...
            clauses[clause_count].constructor_pattern = false;
            ...
            if (clauses[clause_count].otherwise) {
                ...
                bool constructor_guard = false;
                if (param_count == 1) {
                    if (gt[0] == '.') {
                        pointfree_guard = true;
                    } else if (strncmp(gt, "not ", 4) == 0) {
                        const char *negated = gt + 4;
                        while (*negated == ' ' || *negated == '\t') negated++;
                        pointfree_guard =
                            wisp_name_looks_unary_predicate(negated) &&
                            strchr(negated, ' ') == NULL &&
                            strchr(negated, '\t') == NULL;
                    } else if (wisp_name_looks_unary_predicate(gt) &&
                               strchr(gt, ' ') == NULL &&
                               strchr(gt, '\t') == NULL) {
                        pointfree_guard = true;
                        ...
                        pointfree_guard = true;
                    } else if (gt[0] >= 'A' && gt[0] <= 'Z') {
                        constructor_guard = true;
                        for (const char *gc = gt; *gc; gc++) {
                            if (!(isalnum((unsigned char)*gc) ||
                                  *gc == '_' || *gc == '-' || *gc == '?' ||
                                  *gc == '!')) {
                                constructor_guard = false;
                                break;
                            }
                        }
                    }
                ...
                if (constructor_guard) {
                    clauses[clause_count].constructor_pattern = true;
                    SB cond;
                    sb_init(&cond);
                    sb_puts(&cond, "(match ");
                    ...
                    sb_puts(&cond, " with | [");
                    sb_puts(&cond, gt);
                    sb_puts(&cond, "] -> True | _ -> False)");
                    clauses[clause_count].cond = sb_take(&cond);
                } else if (pointfree_guard) {
                    clauses[clause_count].cond =
                        wisp_expand_pointfree_guard(at, gt, params, param_count);
                } else {
                    clauses[clause_count].cond =
        ...
        char *multiline_if = NULL;
        if ((size_t)(body_end - body_start) > 3 &&
            strncmp(body_start, "if ", 3) == 0 &&
            !range_contains(body_start, body_end, " then ") && *p) {
            const char *then_start = p;
            const char *then_end = then_start;
            while (*then_end && *then_end != '\n') then_end++;
            const char *then_text = then_start;
            while (then_text < then_end &&
                   (*then_text == ' ' || *then_text == '\t')) then_text++;
            const char *else_start = *then_end == '\n' ? then_end + 1 : then_end;
            const char *else_end = else_start;
            while (*else_end && *else_end != '\n') else_end++;
            const char *else_text = else_start;
            while (else_text < else_end &&
                   (*else_text == ' ' || *else_text == '\t')) else_text++;
            bool then_line = then_end - then_text >= 5 &&
                strncmp(then_text, "then ", 5) == 0;
            bool else_line = else_end - else_text >= 5 &&
                strncmp(else_text, "else ", 5) == 0;
            if (then_line && else_line) {
                SB conditional;
                sb_init(&conditional);
                for (const char *q = body_start; q < body_end; q++)
                    sb_putc(&conditional, *q);
                sb_putc(&conditional, ' ');
                for (const char *q = then_text; q < then_end; q++)
                    sb_putc(&conditional, *q);
                sb_putc(&conditional, ' ');
                for (const char *q = else_text; q < else_end; q++)
                    sb_putc(&conditional, *q);
                multiline_if = sb_take(&conditional);
                body_start = multiline_if;
                body_end = multiline_if + strlen(multiline_if);
                p = *else_end == '\n' ? else_end + 1 : else_end;
            }
        }
        ...
        sb_init(&body);
        int expr_count = 0;
        SB inline_expression;
        sb_init(&inline_expression);
        bool has_inline_expression = *body_start != '\0' && body_start < body_end;
        if (has_inline_expression) {
            for (const char *q = body_start; q < body_end; q++)
                sb_putc(&inline_expression, *q);
        }
        free(multiline_if);
        ...
        while (*p) {
            ...
            char *joined_if = NULL;
            char *joined_asm = NULL;
            char *joined_nested = NULL;
            if (strncmp(cont_line, "asm ", 4) == 0) {
                SB assembly;
                sb_init(&assembly);
                sb_puts(&assembly, cont_line);

                while (*p) {
                    const char *als = p;
                    const char *ale = p;
                    while (*ale && *ale != '\n') ale++;
                    const char *alnext = *ale == '\n' ? ale + 1 : ale;
                    const char *alt = als;
                    while (alt < ale && (*alt == ' ' || *alt == '\t')) alt++;
                    while (ale > alt &&
                           (ale[-1] == ' ' || ale[-1] == '\t' || ale[-1] == '\r'))
                        ale--;
                    ...
                    if (measure_indent(als) <= 0 ||
                        wisp_find_top_level_arrow(alt))
                        break;

                    sb_putc(&assembly, ' ');
                    for (const char *ac = alt; ac < ale; ac++)
                        sb_putc(&assembly, *ac);
                    p = alnext;
                }

                joined_asm = sb_take(&assembly);
                cont_stmt = joined_asm;
            }
            ...
            if (!has_inline_expression && !joined_asm && !joined_if) {
                SB statement;
                sb_init(&statement);
                sb_puts(&statement, cont_line);
                bool has_nested_lines = false;
                while (*p) {
                    const char *nls = p;
                    const char *nle = p;
                    while (*nle && *nle != '\n') nle++;
                    const char *nlt = nls;
                    while (nlt < nle && (*nlt == ' ' || *nlt == '\t')) nlt++;
                    if (nlt >= nle || *nlt == ';') break;
                    if (measure_indent(nls) <= cont_indent) break;
                    sb_putc(&statement, '\n');
                    for (const char *q = nls; q < nle; q++)
                        sb_putc(&statement, *q);
                    p = *nle == '\n' ? nle + 1 : nle;
                    has_nested_lines = true;
                }
                if (has_nested_lines) {
                    joined_nested = sb_take(&statement);
                    cont_stmt = joined_nested;
                } else {
                    free(sb_take(&statement));
                }
            }
            cont_end = (strncmp(cont_stmt, "asm ", 4) == 0 || joined_nested)
                ? cont_stmt + strlen(cont_stmt)
                : get_logical_line_end(cont_stmt);
            if (has_inline_expression) {
                sb_putc(&inline_expression, ' ');
                for (const char *q = cont_stmt; q < cont_end; q++)
                    sb_putc(&inline_expression, *q);
            } else {
                wisp_append_expanded_statement(at, &body, cont_stmt, cont_end,
                                               &expr_count);
            }
            free(joined_asm);
            free(joined_if);
        ...
        if (has_inline_expression) {
            char *inline_source = sb_take(&inline_expression);
            char *inline_body = wisp_expand_statement_range(
                at, inline_source, inline_source + strlen(inline_source));
            sb_puts(&body, inline_body);
            expr_count++;
            free(inline_body);
            free(inline_source);
        } else {
            free(sb_take(&inline_expression));
        }
    ...
    int last_clause = clause_count - 1;
    char *body = clauses[last_clause].constructor_pattern
        ? strdup(clauses[last_clause].body)
        : strdup("(undefined)");
    if (clauses[last_clause].constructor_pattern)
        last_clause--;
    for (int i = last_clause; i >= 0; i--) {
...
static bool wisp_guard_starts_binary_op(const char *source) {
    if (!source) return false;
    while (*source == ' ' || *source == '\t') source++;
    const char *end = source;
    while (*end && *end != ' ' && *end != '\t') end++;
    char *head = strndup(source, (size_t)(end - source));
    bool result = wisp_guard_is_binary_op(head);
    free(head);
    return result;
}
...
/* Find a word-form Boolean connective before tokenization assigns infix
 * precedence.  Point-free guards apply the matched subject independently to
 * each side, so `>= lo and <= hi` must become
 * `(and (>= subject lo) (<= subject hi))`, never
 * `(>= subject (and lo (<= hi)))`. */
static const char *wisp_guard_find_top_level_logic(const char *start,
                                                   const char *end,
                                                   size_t *logic_len) {
    int depth = 0;
    bool in_string = false;
    ...
    for (const char *p = start; p < end; p++) {
        ...
        const bool left_boundary = p == start || p[-1] == ' ' || p[-1] == '\t';
        if (!left_boundary) continue;
        if (p + 3 <= end && strncmp(p, "and", 3) == 0 &&
            (p + 3 == end || p[3] == ' ' || p[3] == '\t')) {
            *logic_len = 3;
            return p;
        }
        if (p + 2 <= end && strncmp(p, "or", 2) == 0 &&
            (p + 2 == end || p[2] == ' ' || p[2] == '\t')) {
            *logic_len = 2;
            return p;
        }
...
/* Expand receiver-relative field references in unary guards:
 *
 *   sample | .xp < 0
 *
 * becomes the ordinary field expression `sample.xp < 0` before infix and
 * point-free guard expansion. A dot followed by a decimal digit remains a
 * numeric literal, and quoted text is never rewritten. */
static char *wisp_expand_relative_guard_fields(const char *source,
                                               char **subjects,
                                               int subject_count) {
    if (!source || !subjects || subject_count != 1)
        return strdup(source ? source : "");
    ...
    for (const char *p = source; *p; p++) {
        ...
        bool relative_boundary = p == source ||
            p[-1] == ' ' || p[-1] == '\t' ||
            p[-1] == '(' || p[-1] == '[' || p[-1] == '{';
        if (*p == '.' && relative_boundary &&
            (isalpha(next) || next == '_' || next >= 0x80)) {
            sb_puts(&out, subjects[0]);
            sb_putc(&out, '.');
            continue;
        }
    ...
    if (wisp_debug_enabled() && strcmp(source, result) != 0) {
        fprintf(stderr, "[wisp-guard-field] '%s' -> '%s'\n", source, result);
    }
    return result;
}
...
static char *wisp_expand_pointfree_guard(ArityTable *at,
                                         ...
                                         int subject_count) {
    char *relative_guard =
        wisp_expand_relative_guard_fields(guard_src, subjects, subject_count);
    guard_src = relative_guard;
    const char *guard_end = guard_src + strlen(guard_src);

    const char *bare_end = guard_src;
    while (*bare_end && *bare_end != ' ' && *bare_end != '\t') bare_end++;
    while (*bare_end == ' ' || *bare_end == '\t') bare_end++;
    if (*guard_src && *bare_end == '\0' && subject_count == 1 &&
        wisp_name_looks_unary_predicate(guard_src)) {
        SB unary;
        sb_init(&unary);
        sb_putc(&unary, '(');
        sb_puts(&unary, guard_src);
        sb_putc(&unary, ' ');
        sb_puts(&unary, subjects[0]);
        sb_putc(&unary, ')');
        char *result = sb_take(&unary);
        free(relative_guard);
        return result;
    }

    /* Negation preserves receiver-relative predicate shorthand:
     *   value | not ready? -> ...
     * is the readable spelling of `not (ready? value)`. */
    if (subject_count == 1 && strncmp(guard_src, "not ", 4) == 0) {
        const char *predicate = guard_src + 4;
        while (*predicate == ' ' || *predicate == '\t') predicate++;
        const char *predicate_end = predicate;
        while (*predicate_end && *predicate_end != ' ' &&
               *predicate_end != '\t') predicate_end++;
        const char *tail = predicate_end;
        while (*tail == ' ' || *tail == '\t') tail++;
        if (*predicate && *tail == '\0') {
            char *name = strndup(predicate,
                                 (size_t)(predicate_end - predicate));
            if (wisp_name_looks_unary_predicate(name)) {
                SB negated;
                sb_init(&negated);
                sb_puts(&negated, "(not (");
                sb_puts(&negated, name);
                sb_putc(&negated, ' ');
                sb_puts(&negated, subjects[0]);
                sb_puts(&negated, "))");
                free(name);
                free(relative_guard);
                return sb_take(&negated);
            }
            free(name);
        }
    }

    /* A complete binary comparison in guard position is an expression, never
     * a mutable binding. Without handling it before the generic token stream,
     * `item = 0` is lowered as `(var item 0)` because guard snippets begin at
     * column zero. Point-free `= 0` deliberately has no left operand and
     * continues through the implicit-subject path below. */
    size_t comparison_len = 0;
    const char *comparison = wisp_find_top_level_binary_guard_op(
        guard_src, guard_end, &comparison_len);
    if (comparison) {
        const char *lhs_end = comparison;
        while (lhs_end > guard_src &&
               (lhs_end[-1] == ' ' || lhs_end[-1] == '\t'))
            lhs_end--;
        if (lhs_end > guard_src) {
            char *result = wisp_expand_simple_guard_condition(
                at, guard_src, guard_end);
            free(relative_guard);
            return result;
        }
    }
    ...
    size_t logic_len = 0;
    const char *logic = wisp_guard_find_top_level_logic(guard_src, guard_end,
                                                        &logic_len);
    if (logic) {
        char *left_src = strndup(guard_src, (size_t)(logic - guard_src));
        char *right_src = strdup(logic + logic_len);
        char *left = wisp_expand_pointfree_guard(at, left_src,
                                                 subjects, subject_count);
        char *right = wisp_expand_pointfree_guard(at, right_src,
                                                  subjects, subject_count);
        char *op = strndup(logic, logic_len);
        SB combined;
        sb_init(&combined);
        sb_putc(&combined, '(');
        sb_puts(&combined, op);
        sb_putc(&combined, ' ');
        sb_puts(&combined, left);
        sb_putc(&combined, ' ');
        sb_puts(&combined, right);
        sb_putc(&combined, ')');
        free(left_src);
        free(right_src);
        free(left);
    ...
    while (ts.pos < ts.count) {
        ...
        if (wisp_guard_is_binary_op(tok->text)) {
            ...
            if (subject_count > 0) {
                /* Guard operators receive every clause subject that is not
                 * already written as an operand. Missing subjects are applied
                 * from left to right before the explicit operands:
                 *
                 *   left right | =       => (= left right)
                 *   left right | = left  => (= right left)
                 */
                bool *explicit_subject =
                    calloc((size_t)subject_count, sizeof(bool));
                SB operands;
                sb_init(&operands);
                ...
                while (ts.pos < ts.count) {
                    SB operand_out;
                    sb_init(&operand_out);
                    wisp_parse_expr(at, &ts, &operand_out, -1, 1, 0);
                    char *operand = sb_take(&operand_out);
                    for (int i = 0; i < subject_count; i++) {
                        if (strcmp(operand, subjects[i]) == 0)
                            explicit_subject[i] = true;
                    }
                    sb_putc(&operands, ' ');
                    sb_puts(&operands, operand);
                    free(operand);
                }
                ...
                sb_puts(&e, op);
                for (int i = 0; i < subject_count; i++) {
                    if (!explicit_subject[i]) {
                        sb_putc(&e, ' ');
                        sb_puts(&e, subjects[i]);
                    }
                }
                char *operand_text = sb_take(&operands);
                sb_puts(&e, operand_text);
                sb_putc(&e, ')');
                free(operand_text);
                free(explicit_subject);
                expr = sb_take(&e);
            } else {
                SB e;
                sb_init(&e);
            ...
            bool bare_subject =
                tok->text &&
                wisp_guard_bare_is_subject(tok->text, subjects, subject_count) &&
                ts.pos + 1 == ts.count;
            ...
            if (bare_subject) {
                expr = strdup(tok->text);
                ts.pos++;
            } else if (explicit_subject_call) {
                SB e;
                sb_init(&e);
                sb_putc(&e, '(');
                sb_puts(&e, tok->text);
                ...
                if (wisp_guard_bare_is_subject(expanded, subjects,
                                               subject_count)) {
                    expr = strdup(expanded);
                } else {
...
static char *wisp_expand_statement_range(ArityTable *at,
    ...
    if (assign) {
        ...
        char *rhs_src = strndup(rhs_start, (size_t)(end - rhs_start));
        /* A layout pipeline is one RHS even though each pipe starts on a
         * deeper line. Join only pipe-continuation newlines so unrelated
         * nested layout retains its normal statement boundaries. */
        for (char *p = rhs_src; *p; p++) {
            if (*p != '\n' && *p != '\r') continue;
            char *next = p + 1;
            while (*next == ' ' || *next == '\t') next++;
            if (next[0] == '|' && next[1] == '>') {
                *p = ' ';
                for (char *gap = p + 1; gap < next; gap++) *gap = ' ';
            }
        }
        char *rhs = wisp_expand_expr_snippet(at, rhs_src);
        free(rhs_src);
...
static char *wisp_try_build_inferred_define_form(ArityTable *at,
    ...
    while (*scan) {
        ...
        char *joined_conditional = NULL;
        if ((size_t)(rhs_end - rhs_start) > 3 &&
            strncmp(rhs_start, "if ", 3) == 0 &&
            !range_contains(rhs_start, rhs_end, " then ") && *scan) {
            const char *then_start = scan;
            const char *then_end = then_start;
            while (*then_end && *then_end != '\n') then_end++;
            const char *then_text = then_start;
            while (then_text < then_end && isspace((unsigned char)*then_text))
                then_text++;
            const char *else_start = *then_end == '\n' ? then_end + 1 : then_end;
            const char *else_end = else_start;
            while (*else_end && *else_end != '\n') else_end++;
            const char *else_text = else_start;
            while (else_text < else_end && isspace((unsigned char)*else_text))
                else_text++;
            if (then_end - then_text >= 5 &&
                strncmp(then_text, "then ", 5) == 0 &&
                else_end - else_text >= 5 &&
                strncmp(else_text, "else ", 5) == 0) {
                SB joined;
                sb_init(&joined);
                for (const char *q = rhs_start; q < rhs_end; q++)
                    sb_putc(&joined, *q);
                sb_putc(&joined, ' ');
                for (const char *q = then_text; q < then_end; q++)
                    sb_putc(&joined, *q);
                ...
                for (const char *q = else_text; q < else_end; q++)
                    sb_putc(&joined, *q);
                joined_conditional = sb_take(&joined);
                rhs_start = joined_conditional;
                rhs_end = joined_conditional + strlen(joined_conditional);
                scan = *else_end == '\n' ? else_end + 1 : else_end;
                line_no += 2;
            }
        }
        ...
        SB inline_expression;
        sb_init(&inline_expression);
        bool has_inline_expression = *rhs_start != '\0' && rhs_start < rhs_end;
        if (has_inline_expression) {
            for (const char *q = rhs_start; q < rhs_end; q++)
                sb_putc(&inline_expression, *q);
        }
        free(joined_conditional);
        ...
        while (*scan) {
            ...
            if (strncmp(cont_line, "asm ", 4) == 0) {
                ...
                while (*scan) {
                    ...
                    if (measure_indent(als) <= clause_indent ||
                        wisp_find_top_level_arrow(alt))
                        break;
                ...
                free(cont_line);
                cont_line = sb_take(&assembly);
            }

            /* With an empty inline RHS, the first continuation line is the
             * expression head.  Any lines nested beneath that head are its
             * arguments, not additional expressions in the clause body. */
            if (!has_inline_expression && strncmp(cont_line, "asm ", 4) != 0) {
                SB statement;
                sb_init(&statement);
                sb_puts(&statement, cont_line);
                bool has_nested_lines = false;
                ...
                if (has_nested_lines) {
                    free(cont_line);
                    cont_line = sb_take(&statement);
                } else {
                    char *unused_statement = sb_take(&statement);
                    free(unused_statement);
                }
            }

            const char *cont_end = strncmp(cont_line, "asm ", 4) == 0
                ? cont_line + strlen(cont_line)
                : get_logical_line_end(cont_line);
            if (has_inline_expression) {
                /* A clause with an inline expression head owns its deeper
                 * layout lines.  Join them before recursive expansion so
                 * calls whose arguments include a multiline lambda remain
                 * one expression rather than becoming a `begin` sequence. */
                sb_putc(&inline_expression, ' ');
                for (const char *q = cont_line; q < cont_end; q++)
                    sb_putc(&inline_expression, *q);
            } else {
                wisp_append_expanded_statement(at, &body, cont_line, cont_end,
                                               &expr_count);
            }
            free(cont_line);
            line_no++;
        ...
        if (has_inline_expression) {
            char *inline_source = sb_take(&inline_expression);
            if (wisp_debug_enabled())
                fprintf(stderr, "[wisp-inline-body] '%s'\n", inline_source);
            char *inline_expr = wisp_expand_expr_snippet(at, inline_source);
            sb_puts(&body, inline_expr);
            expr_count++;
            free(inline_expr);
            free(inline_source);
        } else {
            char *unused_inline = sb_take(&inline_expression);
            free(unused_inline);
        }
...
static char *desugar_size_array(const char *tok) {
    ...
    if (*after_suffix != ']' && *after_suffix != '\0') {
        ...
        if      (strcmp(elem_type,"I8")  ==0||strcmp(elem_type,"U8")  ==0) elem_size = 1;
                 ...
                 strcmp(elem_type,"F64") ==0||strcmp(elem_type,"Float")==0) elem_size = 8;
...
static char *wisp_rewrite_grouped_infix(ArityTable *t, const char *text) {
    ...
    bool can_rewrite =
        count == 3 &&
        wisp_syntax_is_infix_operator(items[1]) &&
...
static MONAD_THREAD_LOCAL int g_infix_fence = -1; /* stream pos: infix promotion stops before this */
...
static MONAD_THREAD_LOCAL int g_for_iter_depth = 0;
...
static bool wisp_relation_declared_before(const char *source,
                                          const char *limit,
                                          const char *name) {
    const char *p = source;
    size_t name_len = strlen(name);
    while (p && p < limit && (p = strstr(p, "define ")) && p < limit) {
        const char *candidate = p + 7;
        while (candidate < limit && (*candidate == ' ' || *candidate == '\t'))
            candidate++;
        if (strncmp(candidate, name, name_len) == 0 &&
            (candidate[name_len] == ' ' || candidate[name_len] == '\t')) {
            const char *end = strchr(candidate, '\n');
            if (!end || end > limit) end = limit;
            const char *sig = strstr(candidate + name_len, "::");
            const char *open = sig && sig < end ? strchr(sig + 2, '{') : NULL;
            const char *arrow = open && open < end ? strstr(open + 1, "->") : NULL;
            if (arrow && arrow < end)
                return true;
        }
        p = candidate + name_len;
    }
    return false;
}
...
static WTokenStream build_token_stream(const char *source, ArityTable *at) {
    WTokenStream s = {0};
    if (wisp_debug_enabled() && source && strstr(source, "apply-one"))
        fprintf(stderr, "[wisp-build-source] '%s'\n", source);
    const char *p = source;
    int lineno = 1;
    ...
    while (*p) {
        ...
        /* Sentence forms keep their complete predicate application together
         * before quote-aware tokenization. Besides making unary assertion
         * pleasant, this admits the relational reading `show who likes x`,
         * lowered to the ordinary call `show (likes who x)`. */
        {
            const char *text = raw;
            while (*text == ' ' || *text == '\t') text++;
            if (strncmp(text, "assert ", 7) == 0) {
                size_t indent_len = (size_t)(text - raw);
                const char *predicate = text + 7;
                size_t needed = indent_len + strlen(predicate) + 12;
                char *grouped = malloc(needed);
                snprintf(grouped, needed, "%.*sassert (%s)",
                         (int)indent_len, raw, predicate);
                free(raw);
                raw = grouped;
            } else if (strncmp(text, "show ", 5) == 0) {
                char subject[256], relation[256], object[256], extra[2];
                int fields = sscanf(text + 5, "%255s %255s %255s %1s",
                                    subject, relation, object, extra);
                bool prefix_relation = fields == 3 &&
                    wisp_relation_declared_before(source, line_start, subject);
                bool infix_relation = fields == 3 &&
                    wisp_relation_declared_before(source, line_start, relation);
                if (prefix_relation || infix_relation) {
                    size_t indent_len = (size_t)(text - raw);
                    size_t needed = indent_len + strlen(subject) +
                                    strlen(relation) + strlen(object) + 12;
                    char *grouped = malloc(needed);
                    snprintf(grouped, needed, "%.*sshow (%s %s %s)",
                             (int)indent_len, raw,
                             prefix_relation ? subject : relation,
                             prefix_relation ? relation : subject,
                             object);
                    free(raw);
                    raw = grouped;
                }
        ...
        /* A function signature may continue on indented physical lines:
         *
         *   define node ::
         *     a -> e (Tree e a) -> Tree e a
         *
         *   define append-step :: Evaluation e =>
         *     (l -> Step e l a) -> r -> Step e l a
         *     -> Step e (AppendState e l r a) a
         *
         * A trailing declaration marker/context/arrow requires another type
         * segment.  A following line beginning with `->` is likewise an
         * unmistakable continuation.  Join all such segments before normal
         * define elaboration, while leaving the first pattern clause (which
         * has the same indentation) untouched. */
        {
            bool is_define =
                (strncmp(t, "define", 6) == 0 &&
                 (t[6] == ' ' || t[6] == '\t')) ||
                (strncmp(t, "def", 3) == 0 &&
                 (t[3] == ' ' || t[3] == '\t'));
            int define_indent = measure_indent(raw);

            while (is_define && strstr(t, "::") && *p) {
                ...
                while (*next_end && *next_end != '\n')
                    next_end++;
                char *next_raw = strndup(next_start,
                                         (size_t)(next_end - next_start));
                const char *next_text = next_raw;
                while (*next_text == ' ' || *next_text == '\t')
                    next_text++;
                bool begins_return_arrow =
                    next_text[0] == '-' && next_text[1] == '>';

                if ((needs_segment || begins_return_arrow) &&
                    *next_text && *next_text != ';' &&
                    measure_indent(next_raw) > define_indent) {
                    size_t raw_len = strlen(raw);
                    size_t continuation_len = strlen(next_text);
                    char *joined = malloc(raw_len + continuation_len + 2);
                    if (!joined) {
                        free(next_raw);
                        free(raw);
                        abort();
                    }
                    memcpy(joined, raw, raw_len);
                    joined[raw_len] = ' ';
                    memcpy(joined + raw_len + 1, next_text,
                           continuation_len + 1);
                    free(raw);
                    raw = joined;
                    t = raw;
        ...
        /* A qualified effect signature may put its constraint on the next
         * indented physical line:
         *
         *   define map :: (a -e-> b) -> [a] -e-> [b]
         *     where e has io
         *
         * The ordinary reader deliberately owns the grammar for `where`.
         * Wisp's only job here is layout: join the qualification to the
         * signature before the existing define lowering sees it.  This keeps
         * inline and indented spellings on one elaboration path.
         */
        {
            bool is_define =
                (strncmp(t, "define", 6) == 0 &&
                ...
                (strncmp(t, "def", 3) == 0 &&
                 (t[3] == ' ' || t[3] == '\t'));
            bool has_signature = strstr(t, "::") != NULL;
            bool already_qualified = strstr(t, " where ") != NULL;

            if (is_define && has_signature && !already_qualified && *p) {
                const char *next_start = p;
                const char *next_end = next_start;
                while (*next_end && *next_end != '\n')
                    next_end++;
                ...
                if (measure_indent(next_raw) > measure_indent(raw) &&
                    strncmp(next_text, "where ", 6) == 0) {
                    size_t raw_len = strlen(raw);
                    size_t where_len = strlen(next_text);
                    char *joined = malloc(raw_len + where_len + 2);
                    if (!joined) {
                        fprintf(stderr,
                                "[wisp] out of memory joining effect qualification at line %d\n",
                                lineno);
                        free(next_raw);
                        free(raw);
                        abort();
                    ...
                    memcpy(joined, raw, raw_len);
                    joined[raw_len] = ' ';
                    memcpy(joined + raw_len + 1, next_text, where_len + 1);
                    free(raw);
                    raw = joined;
                    t = raw;
                    while (*t == ' ' || *t == '\t')
        ...
        if (strncmp(t, "define", 6) != 0 &&
...

            {
                ...
                if (arrow && !colon_before_arrow) {
                    ...
                    /* Every line deeper than the pattern clause belongs to
                     * its body, even when the inline head is balanced.  Join
                     * those physical lines before recursively expanding the
                     * expression.  Parentheses retain the nested expression
                     * boundary, while the recursive line-oriented tokenizer
                     * must not see an incomplete `(lambda (x)` as a complete
                    ...
                    while (*p) {
                        ...
                        const char *cle = p;
                        if (*p == '\n') p++;
                        char *clraw = strndup(cls, (size_t)(cle - cls));
                        const char *clt = clraw;
                        while (*clt == ' ' || *clt == '\t') clt++;
                        if (!*clt || *clt == ';') { free(clraw); lineno++; continue; }
                        int continuation_indent = measure_indent(clraw);
                        if (continuation_indent <= indent ||
                            (strncmp(clt, "where", 5) == 0 &&
                             (clt[5] == '\0' || clt[5] == ' ' || clt[5] == '\t'))) {
                            p = cls;
                            free(clraw);
                            break;
                        ...
                        size_t clt_len = strlen(clt);
                        while (body_acc_len + clt_len + 3 >= body_acc_cap) {
                            body_acc_cap *= 2; body_acc = realloc(body_acc, body_acc_cap);
                        }
                        body_acc[body_acc_len++] = ' ';
                        memcpy(body_acc + body_acc_len, clt, clt_len);
                        body_acc_len += clt_len;
                        body_acc[body_acc_len] = '\0';
                        free(clraw);
                        lineno++;
                    }
...
#undef EXPAND_WISP
                    ...
                    if (wisp_debug_enabled())
                        fprintf(stderr, "[wisp-arrow-body] '%s'\n", body_raw);
                        ...
                        while (!is_keyword && infix_op_scan < body_ts.count) {
                            ...
                            if (is_atom && (scan_arity >= 2 || scan_arity == -1)) {
                                ...
                                while (body_ts.pos < body_ts.count) {
                                    ...
                                    int op_prec =
                                        wisp_syntax_operator_precedence(op_name);
                                    body_ts.pos++;
                    ...
                    if (inline_pipe) {
                        ...
                        if (*gstart) {
                            ...
                            if (guard_subject_count == 1 ||
                                (guard_subject_count > 1 &&
                                 wisp_guard_starts_binary_op(gstart))) {
                                guard_expanded = wisp_expand_pointfree_guard(at, gstart,
                                                                             guard_subjects,
                                                                             guard_subject_count);
                            } else {
                                /* Complex patterns intentionally provide no
                                 * implicit receiver, but their guard is still
                                 * a condition. Preserve `lhs = rhs` as
                                 * equality instead of letting the ordinary
                                 * statement reader reinterpret it as `var`. */
                                guard_expanded = wisp_expand_guard_without_subjects(
                                    at, gstart);
                            }
                        ...
                        if (where_pos) {
                            ...
                            if (bind_count > 0) {
                                ...
                                if (body_expanded[0] != '(' && body_expanded[0] != '[' &&
                                    body_expanded[0] != '{' &&
                                    !wisp_is_single_token_form(body_expanded)) {
                                    sb_putc(&wb, '(');
        ...
        if (*t == '(' || *t == '[' || *t == '{' ||
            ...
            SB acc; sb_init(&acc);
            bool multiline_lambda =
                strncmp(t, "(lambda", 7) == 0 &&
                (t[7] == ' ' || t[7] == '\t' || t[7] == '(');
            ...
            if (multiline_lambda) {
                const char *params = final_acc + 7;
                while (*params == ' ' || *params == '\t') params++;
                if (*params == '(') {
                    int param_depth = 0;
                    const char *params_end = params;
                    do {
                        if (*params_end == '(') param_depth++;
                        else if (*params_end == ')') param_depth--;
                        params_end++;
                    } while (*params_end && param_depth > 0);
                    const char *body = params_end;
                    while (*body == ' ' || *body == '\t') body++;
                    ...
                    if (body_len > 0 && body[body_len - 1] == ')') {
                        char *body_source = strndup(body, body_len - 1);
                        char *body_expanded =
                            wisp_expand_expr_snippet(at, body_source);
                        SB rebuilt;
                        sb_init(&rebuilt);
                        sb_puts(&rebuilt, "(lambda ");
                        for (const char *q = params; q < params_end; q++)
                            sb_putc(&rebuilt, *q);
                        sb_putc(&rebuilt, ' ');
                        sb_puts(&rebuilt, body_expanded);
                        sb_putc(&rebuilt, ')');
                        free(final_acc);
                        final_acc = sb_take(&rebuilt);
                        free(body_expanded);
                        free(body_source);
                    }
                }
        ...
        if ((strncmp(t, "layout", 6) == 0 &&
            ...
            bool is_layout_block =
                strncmp(t, "layout", 6) == 0 &&
                (t[6] == ' ' || t[6] == '\t' || t[6] == '\0');
            bool is_indexed_data =
                strncmp(t, "data", 4) == 0 &&
                (t[4] == ' ' || t[4] == '\t') &&
                strstr(t, "::") != NULL;
            ...
            while (*p) {
                ...
                if (is_indexed_data) {
                    const char *colon = memchr(lt, ':', (size_t)(lt_end - lt));
                    if (colon) {
                        ...
                        while (name_start < name_end &&
                            ...
                            name_start++;
                        int constructor_arity = 0;
                        int type_depth = 0;
                        for (const char *q = colon + 1; q + 1 < lt_end; q++) {
                            if (*q == '(' || *q == '[' || *q == '{') type_depth++;
                            else if (*q == ')' || *q == ']' || *q == '}') type_depth--;
                            else if (type_depth == 0 && *q == '-' && q[1] == '>') {
                                constructor_arity++;
                                q++;
                            }
                        }
                        if (name_end > name_start) {
                            ...
                            arity_set(at, constructor_name, constructor_arity);
                            free(constructor_name);
                        }
                    }
                }

                if (fat_arrow && !strstr(lt, "::") && !is_indexed_data) {
                    ...
                    if (body_acc_len == 0 || body_acc_trim[0] == '\0') {
                        ...
                        bool already_grouped = (body_expanded[0] == '(' ||
                                                body_expanded[0] == '[' ||
                                                body_expanded[0] == '{');
                        bool single_token = wisp_is_single_token_form(body_expanded);
                    ...
                    if (inline_pipe) {
                        ...
                        } else if (block_guard_subject_count == 1 ||
                                   (block_guard_subject_count > 1 &&
                                    wisp_guard_starts_binary_op(gstart))) {
                            guard_expanded =
                        ...
                        if (pat_part_len == 0) {
                            for (int i = 0; i < block_guard_subject_count; i++)
                                emit_len += strlen(block_guard_subjects[i]) + 1;
                        }
                        ...
                        if (pat_part_len > 0) {
                            ...
                            sb_init(&inherited);
                            for (int i = 0; i < block_guard_subject_count; i++) {
                                if (i) sb_putc(&inherited, ' ');
                                sb_puts(&inherited, block_guard_subjects[i]);
                            }
                            sb_puts(&inherited, " | ");
                            sb_puts(&inherited, guard_emit);
                            char *inherited_text = sb_take(&inherited);
                            snprintf(pat_emit, emit_len, "%s", inherited_text);
                            free(inherited_text);
                        } else {
                    ...
                    bool grouped_guard_head = false;
                    if (is_method_block && !strstr(lt, "::")) {
                        const char *next = p;
                        while (*next) {
                            const char *next_end = next;
                            while (*next_end && *next_end != '\n') next_end++;
                            const char *next_text = next;
                            while (next_text < next_end &&
                                   (*next_text == ' ' || *next_text == '\t'))
                                next_text++;
                            if (next_text < next_end && *next_text != ';') {
                                grouped_guard_head =
                                    measure_indent(next) > l_indent &&
                                    *next_text == '|' &&
                                    (next_text + 1 == next_end ||
                                     next_text[1] == ' ' || next_text[1] == '\t');
                                break;
                            }
                            next = *next_end == '\n' ? next_end + 1 : next_end;
                        }
                    ...
                    if (grouped_guard_head) {
                        char **new_subjects = NULL;
                        int new_subject_count =
                            wisp_extract_guard_subjects(lt, lt_end,
                                                        &new_subjects);
                        if (new_subject_count > 0) {
                            wisp_free_guard_subjects(block_guard_subjects,
                                                     block_guard_subject_count);
                    ...
                    size_t ltlen = lt_end - lt;
                    bool bare_layout_field =
                        is_layout_block && *lt != '[' && strstr(lt, "::") != NULL;
                    size_t line_len = is_method_block ? 1 : (size_t)_ld2len;
                    while (acc_len + line_len + ltlen +
                           (bare_layout_field ? 2 : 0) + 4 >= acc_cap) {
                    ...
                    if (bare_layout_field) acc[acc_len++] = '[';
            ...
            while (*p) {
                ...
                if (fat_arrow || pending_pattern) {
                    ...
                    bool body_single_token =
                        wisp_is_single_token_form(body_core);
                ...
                /* A class default may use the same subject-only guarded
                 * layout as a top-level method:
                 *
                 *   enumFromTo start end
                 *     | predicate -> value
                 *     | otherwise -> fallback
                 *
                 * Fold that visual group into one ordinary class default
                 * instead of appending the subject and guard lines as raw
                 * class tokens. */
                if (!fat_arrow && !strstr(lt, "::") && *lt != '|' &&
                    strncmp(lt, "law", 3) != 0) {
                    const char *next = p;
                    const char *next_end = next;
                    while (*next_end && *next_end != '\n') next_end++;
                    ...
                    while (next_text < next_end &&
                           (*next_text == ' ' || *next_text == '\t'))
                        next_text++;
                    bool followed_by_guard =
                        next_text < next_end &&
                        measure_indent(next) > l_indent &&
                        *next_text == '|' &&
                        (next_text + 1 == next_end || next_text[1] == ' ' ||
                         next_text[1] == '\t');

                    if (followed_by_guard) {
                        const char *method_end = lt;
                        while (method_end < lt_end && *method_end != ' ' &&
                               *method_end != '\t')
                            method_end++;
                        const char *args_start = method_end;
                        while (args_start < lt_end &&
                               (*args_start == ' ' || *args_start == '\t'))
                            args_start++;

                        SB guarded_src;
                        sb_init(&guarded_src);
                        for (const char *c = args_start; c < lt_end; c++)
                            sb_putc(&guarded_src, *c);

                        const char *consume_end = p;
                        while (*consume_end) {
                            const char *gls = consume_end;
                            while (*consume_end && *consume_end != '\n')
                                consume_end++;
                            const char *gle = consume_end;
                            const char *glt = gls;
                            while (glt < gle && (*glt == ' ' || *glt == '\t'))
                                glt++;
                            if (glt >= gle || *glt == ';') {
                                if (*consume_end == '\n') consume_end++;
                                continue;
                            }
                            if (measure_indent(gls) <= l_indent || *glt != '|') {
                                consume_end = gls;
                                break;
                            }
                            sb_putc(&guarded_src, '\n');
                            for (const char *c = glt; c < gle; c++)
                                sb_putc(&guarded_src, *c);
                            if (*consume_end == '\n') consume_end++;
                        }

                        char *guarded_text = sb_take(&guarded_src);
                        char *lambda = wisp_try_expand_simple_where_lambda(
                            at, guarded_text);
                        free(guarded_text);
                        if (lambda) {
                            char **subjects = NULL;
                            int subject_count = wisp_extract_guard_subjects(
                                args_start, lt_end, &subjects);
                            /* The class method pattern already binds these
                             * subjects.  Keep only the generated lambda body;
                             * emitting an immediately-applied closure here
                             * invents captures for the default dictionary
                             * method and can leave a null callee in LLVM IR. */
                            const char *lambda_params_end = strchr(lambda + 8, ')');
                            const char *lambda_body = lambda_params_end
                                ? lambda_params_end + 1 : lambda;
                            while (*lambda_body == ' ' || *lambda_body == '\t')
                                lambda_body++;
                            size_t lambda_body_len = strlen(lambda_body);
                            if (lambda_params_end && lambda_body_len > 0 &&
                                lambda_body[lambda_body_len - 1] == ')')
                                lambda_body_len--;
                            char *body_text = strndup(lambda_body,
                                                     lambda_body_len);

                            size_t need = acc_len + (size_t)line_tag_len +
                                          (size_t)(lt_end - lt) +
                                          strlen(body_text) + 10;
                            while (need >= acc_cap) {
                                acc_cap *= 2;
                            ...
                            memcpy(acc + acc_len, line_tag,
                                   (size_t)line_tag_len);
                            acc_len += (size_t)line_tag_len;
                            acc[acc_len++] = '(';
                            memcpy(acc + acc_len, lt,
                                   (size_t)(lt_end - lt));
                            acc_len += (size_t)(lt_end - lt);
                            memcpy(acc + acc_len, ") => ", 5);
                            acc_len += 5;
                            memcpy(acc + acc_len, body_text,
                                   strlen(body_text));
                            acc_len += strlen(body_text);
                            acc[acc_len] = '\0';

                            int consumed = 0;
                            for (const char *c = p; c < consume_end; c++)
                                if (*c == '\n') consumed++;
                            p = consume_end;
                            lineno += consumed;
                            wisp_free_guard_subjects(subjects, subject_count);
         ...
         * Also handles: "define name [p :: T] [p :: T] -> Ret" bracketed params. */
        {
            ...
            if (_def_len > 0) {
                ...
                const char *after_name = name_end;
                while (*after_name == ' ' || *after_name == '\t') after_name++;
                const char *alias_start = NULL;
                const char *alias_end = NULL;
                if (!(after_name[0] == ':' && after_name[1] == ':')) {
                    const char *signature_sep = strstr(after_name, "::");
                    if (signature_sep) {
                        alias_start = after_name;
                        alias_end = signature_sep;
                        while (alias_end > alias_start &&
                               (alias_end[-1] == ' ' || alias_end[-1] == '\t'))
                            alias_end--;
                        after_name = signature_sep;
                    }
                }
                        ...
                        if (*_inner >= '0' && *_inner <= '9') {
                            ...
                            if (*_after_suf != ']' && *_after_suf != '\0' && *_after_suf != ';') {
                                ...
                                if (_tl > 0 && _tl < sizeof(_elem_type)) {
                                    ...
                                    else if (strcmp(_elem_type,"I64") ==0||strcmp(_elem_type,"U64") ==0||strcmp(_elem_type,"F64")==0||strcmp(_elem_type,"Float")==0) _elem_size = 8;
...
#undef MONAD_STATIC_ARR_WHY
                                ...
                                free(_varname);
                                /* A declaration may put its initialized value on
                                 * the immediately following line:
                                 *
                                 *   define words :: [4 U32]
                                 *   [0x07230203 ...]
                                 *
                                 * Keep the sized annotation and the literal in
                                 * one define.  Previously this path injected []
                                 * eagerly and the real literal became a discarded
                                 * top-level expression. */
                                char *_initializer = NULL;
                                int _initializer_lines = 0;
                                const char *_next = p;
                                while (*_next == ' ' || *_next == '\t') _next++;
                                if (*_next == '[') {
                                    const char *_value_end = skip_balanced_chars(_next);
                                    if (_value_end > _next && _value_end[-1] == ']') {
                                        _initializer = strndup(_next,
                                                               (size_t)(_value_end - _next));
                                        const char *_after_value = _value_end;
                                        while (*_after_value && *_after_value != '\n')
                                            _after_value++;
                                        if (*_after_value == '\n') _after_value++;
                                        for (const char *_q = p; _q < _after_value; _q++)
                                            if (*_q == '\n') _initializer_lines++;
                                        p = _after_value;
                                    }
                                }
                                ...
                                sb_putc(&form, ' ');
                                sb_puts(&form, _initializer ? _initializer : "[]");
                                sb_putc(&form, ')');
                ...
                /* An indented value makes the declaration line
                 * unambiguous: everything after `::` is the type, including
                 * whitespace-applied indices/arguments such as `Vec 3 Int`.
                 * The old inline-value heuristic stopped after `Vec` and
                 * misparsed `3 Int` as extra define operands. */
                if (after_name[0] == ':' && after_name[1] == ':' &&
                    !wisp_find_top_level_arrow(after_name + 2) && *p) {
                    const char *next_start = p;
                    const char *next_end = next_start;
                    while (*next_end && *next_end != '\n') next_end++;
                    char *next_raw = strndup(next_start,
                                             (size_t)(next_end - next_start));
                    const char *next_text = next_raw;
                    while (*next_text == ' ' || *next_text == '\t') next_text++;

                    if (*next_text && *next_text != ';' &&
                        measure_indent(next_raw) > indent) {
                        size_t name_len = (size_t)(name_end - after_define);
                        const char *type_start = after_name + 2;
                        while (*type_start == ' ' || *type_start == '\t')
                            type_start++;
                        const char *type_end = type_start + strlen(type_start);
                        while (type_end > type_start &&
                               (type_end[-1] == ' ' || type_end[-1] == '\t'))
                            type_end--;

                        char *value_tok = wisp_expand_expr_snippet(at, next_text);
                        SB form;
                        sb_init(&form);
                        sb_puts(&form, "(define [");
                        for (size_t i = 0; i < name_len; i++)
                            sb_putc(&form, after_define[i]);
                        sb_puts(&form, " :: ");
                        for (const char *c = type_start; c < type_end; c++)
                            sb_putc(&form, *c);
                        sb_puts(&form, "] ");
                        sb_puts(&form, value_tok);
                        sb_putc(&form, ')');
                        char *form_str = sb_take(&form);
                        wts_push(&s, form_str, indent, lineno);
                        /* A typed relation value is callable with its two
                         * endpoints. Record that surface arity so subsequent
                         * bare Wisp calls group as (relation left right). */
                        if (*type_start == '{' && strstr(type_start, "->")) {
                            char *relation_name = strndup(
                                after_define, (size_t)(name_end - after_define));
                            arity_set(at, relation_name, 2);
                            free(relation_name);
                        }
                ...
                if (after_name[0] == ':' && after_name[1] == ':' &&
                    ...
                    /* A typed value may place its expression on the next
                     * indented line. With no signature arrow and no clause
                     * arrow, this is a constant, not a zero-argument
                     * function:
                     *
                     *   define answer :: Value
                     *     Number 42
                    ...
                    const char *next_end = next_start;
                    while (*next_end && *next_end != '\n') next_end++;
                    const char *next_text = next_start;
                    while (next_text < next_end &&
                           (*next_text == ' ' || *next_text == '\t'))
                        next_text++;
                    if (next_text < next_end &&
                        measure_indent(next_start) > indent &&
                        *next_text != ';' && *next_text != ':' &&
                        !wisp_find_top_level_arrow(next_text)) {
                        const char *next_value_end =
                            get_logical_line_end(next_text);
                        char *value_src = strndup(
                            next_text, (size_t)(next_value_end - next_text));
                        char *value_tok =
                ...
                if (after_name[0] == ':' && after_name[1] == ':' &&
                    (wisp_find_top_level_arrow(after_name + 2) ||
                     sig_is_hole || _sig_is_array_decl)) {
                    size_t name_len = name_end - after_define;
                    ...
                    while (*sp && *sp != ';') {
                        ...
                        const char *labeled_arrow =
                            wisp_labeled_arrow_end(sp, NULL);
                        if (labeled_arrow) {
                            arr_count++;
                            sp = labeled_arrow;
                            sig_param++;
                    ...
                    if (arr_count > 0) {
                        ...
                        if (*body_line) {
                            ...
                            bool has_sibling_clause = false;
                            for (const char *sibling = next_line; *sibling;) {
                                const char *sibling_end = sibling;
                                while (*sibling_end && *sibling_end != '\n')
                                    sibling_end++;
                                const char *sibling_text = sibling;
                                while (sibling_text < sibling_end &&
                                       (*sibling_text == ' ' ||
                                        *sibling_text == '\t'))
                                    sibling_text++;
                                if (sibling_text < sibling_end &&
                                    *sibling_text != ';') {
                                    int sibling_indent = measure_indent(sibling);
                                    if (sibling_indent <= indent)
                                        break;
                                    if (sibling_indent == body_indent &&
                                        wisp_find_top_level_arrow(sibling_text)) {
                                        has_sibling_clause = true;
                                        break;
                                    }
                                }
                                sibling = *sibling_end == '\n'
                                    ? sibling_end + 1 : sibling_end;
                            }
                            ...
                            bool subject_only = body_arrow == NULL && *bt != '\0' &&
                                                *bt != '|' && *bt != ';';
                            if (body_indent > indent &&
                                (body_arrow || subject_only) &&
                                !has_sibling_clause) {
                                const char *left_end = body_arrow
                                                           ? body_arrow
                                                           : bt + strlen(bt);
                                char *left = wisp_trim_range_dup(bt, left_end);
                                char *names[WISP_MAX_PARAMS] = {0};
                                const char *guard_pipe =
                                    body_arrow
                                        ? wisp_find_top_level_pipe_range(bt, body_arrow)
                                        : NULL;
                                if ((guard_pipe || subject_only) &&
                                    arr_count < WISP_MAX_PARAMS) {
                                    char *guard_params =
                                        wisp_trim_range_dup(bt,
                                                            guard_pipe
                                                                ? guard_pipe
                                                                : left_end);
                                    if (wisp_parse_simple_param_names(guard_params,
                                                                      arr_count,
                                                                      names)) {
                                        /* Clause bodies are expanded before the
                                         * final typed header is emitted.  Mark
                                         * parameters as values now so a body
                                         * such as `id count` remains
                                         * `(id count)`, rather than being
                                         * mistaken for a nullary `(count)`
                                         * call. */
                                        ...
                                        if (lambda) {
                                            ...
                                            char *effect_names[WISP_MAX_PARAMS] = {0};
                                            char *ret_type = NULL;
                                            if (wisp_split_arrow_signature(sig_rest, arr_count,
                                                                           param_types, effect_names,
                                                                           &ret_type)) {
                                                ...
                                                /* Keep the typed definition
                                                 * atomic at the Wisp/reader
                                                 * boundary. Emitting three
                                                 * sibling tokens leaves the
                                                 * function header with no
                                                 * body because `define` owns
                                                 * its complete syntax form. */
                                                SB definition;
                                                sb_init(&definition);
                                                sb_puts(&definition, "(define ");
                                                sb_puts(&definition, header);
                                                sb_putc(&definition, ' ');
                                                sb_puts(&definition, body_call);
                                                if (alias_start) {
                                                    const char *a = alias_start;
                                                    while (a < alias_end) {
                                                        while (a < alias_end && (*a == ' ' || *a == '\t')) a++;
                                                        const char *ae = a;
                                                        while (ae < alias_end && *ae != ' ' && *ae != '\t') ae++;
                                                        if (ae > a) {
                                                            sb_puts(&definition, " :alias ");
                                                            for (const char *c = a; c < ae; c++) sb_putc(&definition, *c);
                                                        }
                                                        a = ae;
                                                    }
                                                }
                                                sb_putc(&definition, ')');
                                                char *complete_definition =
                                                    sb_take(&definition);
                                                wts_push(&s,
                                                         complete_definition,
                                                         indent, lineno);
                                                free(complete_definition);
                                ...
                                /* A subject-only line (`patterns` followed by
                                 * indented guards) has no inline RHS arrow.
                                 * It is handled only by the guarded-clause
                                 * expansion above. Never feed it to the simple
                                 * inline-body path, which requires body_arrow. */
                                if (body_arrow && arr_count < WISP_MAX_PARAMS &&
                                    ...
                                    if (wisp_split_arrow_signature(sig_rest, arr_count,
                                        ...
                                        wisp_validate_ignored_signature_names(param_types,
                                                                              ...
                                                                              lineno);
                                        for (int i = 0; i < arr_count; i++) {
                                            /* A malformed or partially recovered
                                             * parameter row may leave a slot empty.
                                             * The prescan is advisory: never turn
                                             * missing arity evidence into an invalid
                                             * pointer walk. The real parser will
                                             * diagnose the source construct. */
                                            if (!names[i]) continue;
                                        ...
                                        if (rhs_src[0] == '\0' && *next_line) {
                                            const char *als = next_line;
                                            const char *ale = als;
                                            while (*ale && *ale != '\n') ale++;
                                            const char *alt = als;
                                            while (alt < ale && (*alt == ' ' || *alt == '\t')) alt++;
                                            ...
                                            if (ale - alt >= 4 &&
                                                ...
                                                while (*asm_scan) {
                                                    ...
                                                    if (clt >= cle ||
                                                        measure_indent(cls) <= body_indent ||
                                                        wisp_find_top_level_arrow(clt))
                                                        break;

                                                    sb_putc(&assembly, ' ');
                                                    for (const char *ac = clt; ac < cle; ac++)
                                                        sb_putc(&assembly, *ac);
                                                    asm_scan = clnext;
                                                }

                                                free(rhs_src);
                                                rhs_src = sb_take(&assembly);
                                                next_line = asm_scan;
                                                consume_end = asm_scan;
                                            }
                                        }

                                        if (strncmp(rhs_src, "asm ", 4) == 0) {
                                            SB assembly;
                                            sb_init(&assembly);
                                            sb_puts(&assembly, rhs_src);
                                            const char *asm_scan = next_line;
                                        ...
                                        /* With an empty inline clause body, the first
                                         * indented line starts the expression.  Lines
                                         * deeper than that first line are its hanging
                                         * arguments, not sibling expressions:
                                         *
                                         *   value ->
                                         *     branch value
                                         *       (lambda (_) left)
                                         *       (lambda (_) right)
                                         *
                                         * Preserve that layout as one snippet.  A later
                                         * line at the first body's indentation remains
                                         * a genuine sequential expression. */
                                        if (rhs_src[0] == '\0' && *next_line) {
                                            const char *first_start = next_line;
                                            const char *first_end = first_start;
                                            while (*first_end && *first_end != '\n')
                                                first_end++;
                                            const char *first_text = first_start;
                                            while (first_text < first_end &&
                                                   (*first_text == ' ' || *first_text == '\t'))
                                                first_text++;
                                            int first_indent = measure_indent(first_start);
                                            if (first_text < first_end &&
                                                first_indent > indent && *first_text != ';') {
                                                const char *group_end =
                                                    *first_end == '\n' ? first_end + 1 : first_end;
                                                const char *scan = group_end;
                                                while (*scan) {
                                                    const char *line_end = scan;
                                                    while (*line_end && *line_end != '\n')
                                                        line_end++;
                                                    const char *line_text = scan;
                                                    while (line_text < line_end &&
                                                           (*line_text == ' ' || *line_text == '\t'))
                                                        line_text++;
                                                    if (line_text == line_end || *line_text == ';') {
                                                        group_end = *line_end == '\n'
                                                            ? line_end + 1 : line_end;
                                                        scan = group_end;
                                                        continue;
                                                    }
                                                    if (measure_indent(scan) <= first_indent)
                                                        break;
                                                    group_end = *line_end == '\n'
                                                        ? line_end + 1 : line_end;
                                                    scan = group_end;
                                                ...
                                                free(rhs_src);
                                                rhs_src = strndup(first_text,
                                                    (size_t)(group_end - first_text));
                                                next_line = group_end;
                                                consume_end = group_end;
                                            }
                                        }
                                        ...
                                        while (*cont_scan) {
                                            ...
                                            if (strncmp(rhs_src, "asm ", 4) == 0) {
                                                size_t rhs_len = strlen(rhs_src);
                                                size_t cont_len = strlen(cont_line);
                                                rhs_src = realloc(rhs_src,
                                                                  rhs_len + cont_len + 2);
                                                rhs_src[rhs_len] = ' ';
                                                memcpy(rhs_src + rhs_len + 1,
                                                       cont_line,
                                                       cont_len + 1);
                                                free(cont_line);
                                                consume_end = after_cont;
                                            ...
                                            /* A non-empty inline RHS owns every
                                             * more-indented layout line as part
                                             * of the same expression.  This is
                                             * how an application can receive a
                                             * multiline lambda (or any other
                                             * grouped argument).  Splitting
                                             * these lines into a `begin` turns
                                             * the lambda header into an
                                             * independent malformed form. */
                                            if (rhs_src[0] != '\0') {
                                                size_t rhs_len = strlen(rhs_src);
                                                size_t cont_len = (size_t)(cle - cls);
                                                rhs_src = realloc(rhs_src,
                                                                  rhs_len + cont_len + 2);
                                                rhs_src[rhs_len] = '\n';
                                                memcpy(rhs_src + rhs_len + 1,
                                                       cls, cont_len);
                                                rhs_src[rhs_len + cont_len + 1] = '\0';
                                                free(cont_line);
                                                consume_end = after_cont;
                                                cont_scan = after_cont;
                                                continue;
                                            ...
                                            if (!have_seq_body) {
                                                ...
                                                if (rhs_src[0] != '\0') {
                                                    ...
                                                    wisp_append_expanded_statement(
                                                        at, &seq_body, rhs_src,
                                                        rhs_stmt_end,
                                                        &seq_expr_count);
                                                }
                                        ...
                                        if (alias_start) {
                                            ...
                                            while (a < alias_end) {
                                                ...
                                                if (ae > a) {
                                                    sb_puts(&form, " :alias ");
                                                    for (const char *c = a; c < ae; c++) sb_putc(&form, *c);
                                                }
                                                a = ae;
                                            }
                                        }
                    ...
                    if (arr_count > 0) {
                        ...
                        if (arr_count < WISP_MAX_PARAMS &&
                            ...
                            for (int i = 0; i < arr_count; i++) {
                                ...
                                const char *ptype = param_types[i];
                                while (*ptype == ' ' || *ptype == '\t') ptype++;
                                char *signature_param_name = NULL;
                                char *signature_param_type = NULL;
                                size_t ptype_len = strlen(ptype);
                                if (ptype_len >= 3 && ptype[0] == '[' &&
                                    ptype[ptype_len - 1] == ']') {
                                    const char *colon = strchr(ptype + 1, ':');
                                    if (colon && colon < ptype + ptype_len - 1) {
                                        signature_param_name =
                                            wisp_trim_range_dup(ptype + 1, colon);
                                        ...
                                        signature_param_type =
                                            wisp_trim_range_dup(
                                                type_start,
                                                ptype + ptype_len - 1);
                                        ptype = signature_param_type;
                                    }
                                }
                                const char *param_name = signature_param_name
                                    ? signature_param_name
                                ...
                                if (ptype[0] == '.') {
                                    ...
                                    free(param_types[i]);
                                    free(effect_names[i]);
                                    free(signature_param_name);
                                    free(signature_param_type);
                                    continue;
                                }
                                sb_puts(&hdr, " [");
                        ...
                        for (const char *q = block_start; q < block_end;) {
                            ...
                            if (lt < le && *lt != ';') {
                                ...
                                if (arrow) {
                                    ...
                                    if (body_src[0] != '\0') {
                                        int clause_indent = measure_indent(ls);
                                        const char *body_scan =
                                            (q < block_end && *q == '\n') ? q + 1 : q;
                                        SB joined_body;
                                        sb_init(&joined_body);
                                        sb_puts(&joined_body, body_src);

                                        while (body_scan < block_end) {
                                            const char *body_line_start = body_scan;
                                            while (body_scan < block_end &&
                                                   *body_scan != '\n')
                                                body_scan++;
                                            const char *body_line_end = body_scan;
                                            const char *body_text = body_line_start;
                                            while (body_text < body_line_end &&
                                                   (*body_text == ' ' ||
                                                    *body_text == '\t'))
                                                body_text++;

                                            if (body_text < body_line_end &&
                                                *body_text != ';' &&
                                                measure_indent(body_line_start) <=
                                                    clause_indent) {
                                                body_scan = body_line_start;
                                                break;
                                            }
                                            if (body_text < body_line_end &&
                                                *body_text != ';') {
                                                ...
                                                for (const char *c = body_text;
                                                     c < body_line_end; c++)
                                                    sb_putc(&joined_body, *c);
                                            }
                                            if (body_scan < block_end &&
                                                *body_scan == '\n')
                                                body_scan++;
                                        }

                                        free(body_src);
                                        body_src = sb_take(&joined_body);
                                        if (wisp_debug_enabled())
                                            fprintf(stderr, "[wisp-clause-body] '%s'\n",
                                                    body_src);
                                        q = body_scan;
                                    }
                                    if (body_src[0] == '\0') {
                                            ...
                                            if (body_text < body_line_end &&
                                                *body_text != ';') {
                                                const char *next_line =
                                                    (body_scan < block_end &&
                                                     *body_scan == '\n')
                                                    ? body_scan + 1 : body_scan;
                                                const char *then_end = next_line;
                                                while (then_end < block_end &&
                                                       *then_end != '\n')
                                                    then_end++;
                                                const char *then_text = next_line;
                                                while (then_text < then_end &&
                                                       (*then_text == ' ' ||
                                                        *then_text == '\t'))
                                                    then_text++;
                                                const char *else_line =
                                                    (then_end < block_end &&
                                                     *then_end == '\n')
                                                    ? then_end + 1 : then_end;
                                                const char *else_end = else_line;
                                                while (else_end < block_end &&
                                                       *else_end != '\n')
                                                    else_end++;
                                                const char *else_text = else_line;
                                                while (else_text < else_end &&
                                                       (*else_text == ' ' ||
                                                        *else_text == '\t'))
                                                    else_text++;
                                                bool layout_if =
                                                    body_line_end - body_text > 3 &&
                                                    strncmp(body_text, "if ", 3) == 0 &&
                                                    !range_contains(body_text,
                                                                    body_line_end,
                                                                    " then ") &&
                                                    then_end - then_text >= 5 &&
                                                    strncmp(then_text, "then ", 5) == 0 &&
                                                    else_end - else_text >= 5 &&
                                                    strncmp(else_text, "else ", 5) == 0;
                                                if (layout_if) {
                                                    ...
                                                    for (const char *c = body_text;
                                                        ...
                                                        sb_putc(&joined, *c);
                                                    sb_putc(&joined, ' ');
                                                    for (const char *c = then_text;
                                                         c < then_end; c++)
                                                        sb_putc(&joined, *c);
                                                    sb_putc(&joined, ' ');
                                                    for (const char *c = else_text;
                                                         c < else_end; c++)
                                                        sb_putc(&joined, *c);
                                                    char *joined_text = sb_take(&joined);
                                                    wisp_append_expanded_statement(
                                                        at, &body, joined_text,
                                                        joined_text + strlen(joined_text),
                                                        &body_count);
                                                    free(joined_text);
                                                    body_scan = else_end;
                                                } else {
                                                    ...
                                                    while (nested_scan < block_end) {
                                                        const char *nls = nested_scan;
                                                        while (nested_scan < block_end &&
                                                               *nested_scan != '\n')
                                                            nested_scan++;
                                                        const char *nle = nested_scan;
                                                        const char *nlt = nls;
                                                        while (nlt < nle &&
                                                               (*nlt == ' ' || *nlt == '\t'))
                                                            nlt++;
                                                        if (nlt < nle && *nlt != ';' &&
                                                            measure_indent(nls) <= head_indent) {
                                                            nested_scan = nls;
                                                            break;
                                                        }
                                                        if (nlt < nle && *nlt != ';') {
                                                            sb_putc(&statement, '\n');
                                                            for (const char *c = nls;
                                                                 c < nle; c++)
                                                                sb_putc(&statement, *c);
                                                            has_nested = true;
                                                        }
                                                        if (nested_scan < block_end &&
                                                            *nested_scan == '\n')
                                                            nested_scan++;
                                                    }
                                                    char *statement_text =
                                                        sb_take(&statement);
                                                    wisp_append_expanded_statement(
                                                        at, &body, statement_text,
                                                        statement_text + strlen(statement_text),
                                                        &body_count);
                                                    free(statement_text);
                                                    if (has_nested)
                                                        body_scan = nested_scan;
                                                }
                                            }
                                        ...
                                        if (body_count > 0) {
                                            char *statements = sb_take(&body);
                                            SB sequence;
                                            sb_init(&sequence);
                                            sb_puts(&sequence, "(begin ");
                                            sb_puts(&sequence, statements);
                                            sb_putc(&sequence, ')');
                                            free(statements);
                                            free(body_src);
                                            body_src = sb_take(&sequence);
                                            q = body_scan;
                                        } else {
                                            free(sb_take(&body));
                                        }
                                    ...
                                    if (body_needs_expand) {
                                        body_expanded = wisp_expand_expr_snippet(at, body_src);
                                               ...
                                               body_src[0] != '[' &&
                                               body_src[0] != '{' &&
                                               !wisp_is_single_token_form(body_src)) {
                                    ...
                                    if (pipe) {
                                        ...
                                        if (wisp_line_is_otherwise(guard_start, lhs_end)) {
                                            guard_expanded = strdup("True");
                                                   ...
                                                   (block_guard_subject_count > 1 &&
                                                    wisp_guard_starts_binary_op(guard_start))) {
                                    ...
                                    /* A subject-only line introduces the
                                     * receiver for the indented guard clauses:
                                     *
                                     *   value
                                     *     | >= lower -> ...
                                     *
                                     * Keep emitting the line as the clause
                                     * pattern, and retain its simple subjects
                                     * so every following point-free guard can
                                     * be expanded relative to them. */
                                    const char *subject_start = clause_line;
                                    while (*subject_start == ' ' || *subject_start == '\t')
                                        subject_start++;
                                    if (*subject_start != '|') {
                                        char **new_subjects = NULL;
                                        int new_subject_count =
                                            wisp_extract_guard_subjects(
                                                subject_start,
                                                subject_start + strlen(subject_start),
                                                &new_subjects);
                                        if (new_subject_count > 0) {
                                            wisp_free_guard_subjects(
                                                block_guard_subjects,
                                                block_guard_subject_count);
                                            block_guard_subjects = new_subjects;
                                            block_guard_subject_count =
                ...
                if (after_name[0] == '-' && after_name[1] == '>') {
                    ...
                    while (*scan_pos) {
                        ...
                        SB inline_expression;
                        sb_init(&inline_expression);
                        bool has_inline_expression = false;
                        ...
                        char *joined_if = NULL;
                        if (rhs_end - rhs_start > 3 &&
                            strncmp(rhs_start, "if ", 3) == 0 &&
                            !range_contains(rhs_start, rhs_end, " then ") && *scan_pos) {
                            const char *tls = scan_pos;
                            const char *tle = tls;
                            while (*tle && *tle != '\n') tle++;
                            const char *tt = tls;
                            while (tt < tle && isspace((unsigned char)*tt)) tt++;
                            const char *els = *tle == '\n' ? tle + 1 : tle;
                            const char *ele = els;
                            while (*ele && *ele != '\n') ele++;
                            const char *et = els;
                            while (et < ele && isspace((unsigned char)*et)) et++;
                            if (tle - tt >= 5 && strncmp(tt, "then ", 5) == 0 &&
                                ele - et >= 5 && strncmp(et, "else ", 5) == 0) {
                                char *condition_source = strndup(
                                    rhs_start + 3,
                                    (size_t)(rhs_end - (rhs_start + 3)));
                                char *then_source = strndup(
                                    tt + 5, (size_t)(tle - (tt + 5)));
                                char *else_source = strndup(
                                    et + 5, (size_t)(ele - (et + 5)));
                                char *condition = wisp_expand_expr_snippet(
                                    at, condition_source);
                                char *consequent = wisp_expand_expr_snippet(
                                    at, then_source);
                                char *alternative = wisp_expand_expr_snippet(
                                    at, else_source);
                                SB joined;
                                sb_init(&joined);
                                sb_puts(&joined, "(if ");
                                sb_puts(&joined, condition);
                                sb_putc(&joined, ' ');
                                sb_puts(&joined, consequent);
                                sb_putc(&joined, ' ');
                                sb_puts(&joined, alternative);
                                sb_putc(&joined, ')');
                                joined_if = sb_take(&joined);
                                free(condition_source);
                                free(then_source);
                                free(else_source);
                                free(condition);
                                free(consequent);
                                free(alternative);
                                rhs_start = joined_if;
                                rhs_end = joined_if + strlen(joined_if);
                                scan_pos = *ele == '\n' ? ele + 1 : ele;
                                scan_lineno += 2;
                            }
                        }

                        if (*rhs_start != '\0' && rhs_start < rhs_end) {
                            for (const char *q = rhs_start; q < rhs_end; q++)
                                sb_putc(&inline_expression, *q);
                            has_inline_expression = true;
                        }
                        free(joined_if);
                        ...
                        while (*scan_pos) {
                            ...
                            if (strncmp(body_line, "asm ", 4) == 0) {
                                SB assembly;
                                sb_init(&assembly);
                                sb_puts(&assembly, body_line);

                                while (*scan_pos) {
                                    const char *als = scan_pos;
                                    const char *ale = scan_pos;
                                    while (*ale && *ale != '\n') ale++;
                                    const char *alnext = *ale == '\n' ? ale + 1 : ale;
                                    const char *alt = als;
                                    while (alt < ale && (*alt == ' ' || *alt == '\t')) alt++;
                                ...
                                free(body_line);
                                body_line = sb_take(&assembly);
                            }
                            const char *body_line_end =
                                strncmp(body_line, "asm ", 4) == 0
                                    ? body_line + strlen(body_line)
                                    : get_logical_line_end(body_line);
                            ...
                            if (body_line < body_line_end) {
                                if (has_inline_expression) {
                                    sb_putc(&inline_expression, ' ');
                                    for (const char *q = body_line;
                                         q < body_line_end; q++)
                                        sb_putc(&inline_expression, *q);
                                } else {
                                    char *line_src = strndup(
                                        body_line,
                                        (size_t)(body_line_end - body_line));
                                    char *line_expr = wisp_expand_expr_snippet(
                                        at, line_src);
                                    free(line_src);
                        ...
                        if (has_inline_expression) {
                            char *inline_source = sb_take(&inline_expression);
                            char *inline_expr = wisp_expand_expr_snippet(
                                at, inline_source);
                            sb_puts(&body, inline_expr);
                            body_count++;
                            free(inline_expr);
                ...
                if (*after_name && *after_name != ';') {
                    ...
                    if (value_start < value_end) {
                        if (strncmp(value_start, "::", 2) == 0) {
                            ...
                            /* A type-only line introduces an indented value:
                             *
                             *   define page :: HTML
                             *     html ...
                             *
                             * Keep `define` and its annotated name open on
                             * the Wisp stack so the following expression is
                             * its value. Emitting `[]` here used to fabricate
                             * an empty-array value and detach the body. */
                            SB annotation;
                            sb_init(&annotation);
                            sb_putc(&annotation, '[');
                            sb_puts(&annotation, fname);
                            sb_puts(&annotation, " :: ");
                            sb_puts(&annotation, type_tok);
                            sb_putc(&annotation, ']');
                            char *annotated_name = sb_take(&annotation);
                            wts_push(&s, "define", indent, lineno);
                            wts_push(&s, annotated_name, indent, lineno);
            ...
            if (inline_then) {
                ...
                if (inline_else) {
                    ...
                    if (after_else < if_line_end) {
                        ...
                        /* An inline `else` may introduce an indented body on
                         * following lines, just like a standalone `else`.
                         * Do not feed the empty tail to the expression
                         * expander: its incomplete-input sentinel is not an
                         * AST node. */
                        ...
                        while (*p) {
                            ...
                            while (*p && *p != '\n') p++;
                            if (*p == '\n') p++;
                            char *lraw = strndup(ls, p - ls);
                            const char *lt = lraw;
                            while (*lt==' '||*lt=='\t') lt++;
                            if (!*lt || *lt==';') {
                                free(lraw); lineno++; continue;
                            }
                            int li = measure_indent(lraw);
                            if (li <= indent) {
                                p = ls;
                                free(lraw);
                                break;
                            }
                            if (else_layout_base < 0)
                                else_layout_base = li;
                            int relative_indent = li - else_layout_base;
                            const char *le = get_logical_line_end(lt);
                            size_t content_len = (size_t)(le - lt);
                            char *layout_line = malloc((size_t)relative_indent +
                                                       content_len + 1);
                            memset(layout_line, ' ', (size_t)relative_indent);
                            memcpy(layout_line + relative_indent, lt, content_len);
                            layout_line[relative_indent + content_len] = '\0';
                            if (else_count >= else_cap) {
                                else_cap = else_cap ? else_cap * 2 : 4;
                                else_lines = realloc(else_lines,
                                    sizeof(char *) * (size_t)else_cap);
                            }
                ...
                while (*p) {
                    ...
                    /* A layout `then` may align with `if` or sit inside its
                     * indented branch, matching the accepted `else` shape. */
                    if (li2 >= indent &&
         ...
         * when the next meaningful line is a deeper guard clause. */
        {
            ...
            while (*look) {
                ...
                int l_indent = measure_indent(lraw);
                followed_by_guard = (l_indent > indent && *lt == '|' && lt[1] != '>');
                free(lraw);
                break;
            }
...
static void wisp_parse_expr(ArityTable *t, WTokenStream *s, SB *out, int parent_indent, int parent_remaining, int caller_prec) {
    ...
    bool until_has_block = false;
    if (!is_grouped && strcmp(text, "until") == 0) {
        for (int i = s->pos; i < s->count; i++) {
            if (s->tokens[i].lineno == my_lineno) continue;
            until_has_block = s->tokens[i].indent > my_indent;
            break;
        }
    }
    ...
    if (!is_grouped && strcmp(text, "error") == 0) {
    ...
    else if (!is_grouped && strcmp(text, "until") == 0 && until_has_block) {
        /* Wisp's block form is an imperative loop, distinct from the
         * parenthesized polymorphic Function.until call.  Header-provided
         * predicate arity still determines how much of the condition line is
         * consumed before the indented loop body begins. */
        sb_puts(&prefix_sb, "(while (not ");
        wisp_parse_expr(t, s, &prefix_sb, my_indent, 1, 0);
        sb_putc(&prefix_sb, ')');
        while (s->pos < s->count && s->tokens[s->pos].indent > my_indent) {
            sb_putc(&prefix_sb, '\n');
        ...
        if (!is_grouped && arity == -2 && caller_prec < 6 &&
            wisp_syntax_token_can_call_group(text) &&
            s->pos < s->count &&
            ...
            if (is_grouped && text[0] == '(' &&
                !wisp_syntax_group_contains_quote(text)) {
        ...
        sb_putc(&prefix_sb, '(');
        sb_puts(&prefix_sb, strcmp(text, "assert") == 0
                              ? "assert-eq" : text);
        ...
        if (arity > 0) {
            ...
            for (int i = 0; i < surface_arity && s->pos < s->count; i++) {
                ...
                if (kind == PARAM_FUNC || args_are_bare || preserve_later_fixed_slots) {
                    ...
                    /* The final value slot owns the remaining infix
                     * expression on its line: `show 3 + 4` is
                     * `(show (+ 3 4))`, not `(+ (show 3) 4)`. Earlier slots
                     * stay tight so fixed-arity calls retain their boundary. */
                    int argument_precedence =
                        i == surface_arity - 1 ? 0 : 6;
                    wisp_parse_expr(t, s, &prefix_sb, my_indent, 1,
                                    argument_precedence);
                }
            }
        ...
        if (strcmp(text, "assert") == 0)
            sb_puts(&prefix_sb, " True \"assertion\"");
        sb_putc(&prefix_sb, ')');
    }
    ...
    while (s->pos < s->count && (g_infix_fence == -1 || s->pos < g_infix_fence)) {
        ...
        bool continued_pipe = strcmp(op_tok->text, "|>") == 0 &&
                              op_tok->lineno != my_lineno &&
                              op_tok->indent > my_indent;
        if (op_tok->lineno != my_lineno && !continued_pipe) break;
        bool op_atom = (op_tok->text[0] != '(' &&
                        op_tok->text[0] != '[' &&
                        !(op_tok->text[0] == '~' && op_tok->text[1] == '[') &&
                        op_tok->text[0] != '{');
        if (!op_atom) break;
        bool is_pipe = strcmp(op_tok->text, "|>") == 0;
        int op_ar = wisp_lookup_arity(t, op_tok->text);
        if (op_ar == -2 && wisp_syntax_is_infix_operator(op_tok->text))
            op_ar = 2;
        if (!is_pipe && op_ar < 2 && op_ar != -1) break;

        int op_prec = wisp_syntax_operator_precedence(op_tok->text);
        if (op_prec <= caller_prec) break;

        char *op_name = strdup(op_tok->text);
        bool op_is_right_associative =
        ...
        if (is_pipe) {
            ...
            sb_init(&target);
            /*
             * The token after a pipe denotes the receiver, not an ordinary
             * prefix call.  In particular, a known unary function must remain
             * `double` in `x |> double`; letting arity-driven parsing run here
             * would consume the following pipe as double's argument.
             * Parenthesized partial applications are already one token.
             */
            if (s->pos < s->count &&
                s->tokens[s->pos].lineno == op_tok->lineno) {
                sb_puts(&target, s->tokens[s->pos].text);
                s->pos++;
            }
            char *target_expr = sb_take(&target);
            int target_arity = wisp_lookup_arity(t, target_expr);
            if (wisp_syntax_is_infix_operator(target_expr))
                target_arity = 2;

            SB piped;
            sb_init(&piped);
            sb_putc(&piped, '(');
            sb_puts(&piped, target_expr);
            sb_putc(&piped, ' ');
            sb_puts(&piped, accum);
            /* An infix operator after a pipe is a left section:
             *   value |> * scale  ==>  (* value scale)
             * Ordinary unary targets consume no additional operand. */
            for (int r = 1; r < target_arity && s->pos < s->count; r++) {
                if (s->tokens[s->pos].lineno != op_tok->lineno) break;
                sb_putc(&piped, ' ');
                wisp_parse_expr(t, s, &piped, op_tok->indent, 1,
                                wisp_syntax_operator_precedence(target_expr));
            }
            sb_putc(&piped, ')');
            free(target_expr);
            free(op_name);
        ...
        for (int r = 0; r < rhs_slots && s->pos < s->count; r++) {
            ...
            sb_putc(&next_acc, ' ');
            int rhs_prec = op_is_right_associative ? op_prec - 1 : op_prec;
            wisp_parse_expr(t, s, &next_acc, my_indent, 1, rhs_prec);
        }
        sb_putc(&next_acc, ')');
        accum = sb_take(&next_acc);
    }
...
static char *strip_comments(const char *source) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        /* Column-one `| prose` is a documentation line.  Indented pipes are
         * deliberately retained because they introduce guarded clauses. */
        if (c == '|' && (i == 0 || out[i - 1] == '\n') &&
            (out[i + 1] == ' ' || out[i + 1] == '\t' ||
             out[i + 1] == '\r' || out[i + 1] == '\n' ||
             out[i + 1] == '\0')) {
...
/* Join only the structural newlines of the canonical conditional layout:
 *
 *   if predicate
 *      then consequent
 *       else alternative
 *
 * Keeping this normalization ahead of every definition-lowering path gives
 * inline and multiline conditionals one parser and one AST. Bytes are replaced
 * in place, so all later source offsets remain stable. */
static void join_multiline_conditionals_in_place(char *source) {
    for (char *line = source; line && *line;) {
        char *line_end = strchr(line, '\n');
        if (!line_end) break;
        char *text = line;
        while (text < line_end && (*text == ' ' || *text == '\t')) text++;
        bool conditional =
            (strncmp(text, "if ", 3) == 0 || strstr(text, "-> if ") != NULL) &&
        ...
        char *then_line = line_end + 1;
        char *then_end = strchr(then_line, '\n');
        if (!then_end) break;
        char *then_text = then_line;
        while (then_text < then_end &&
               (*then_text == ' ' || *then_text == '\t')) then_text++;

        char *else_line = then_end + 1;
        char *else_end = strchr(else_line, '\n');
        if (!else_end) else_end = else_line + strlen(else_line);
        char *else_text = else_line;
        while (else_text < else_end &&
               (*else_text == ' ' || *else_text == '\t')) else_text++;

        bool then_branch = then_end - then_text >= 5 &&
            strncmp(then_text, "then ", 5) == 0;
        bool else_branch = else_end - else_text >= 5 &&
            strncmp(else_text, "else ", 5) == 0;
...
static void wisp_normalize_type_definition_equals(char *source) {
    for (char *line = source; line && *line;) {
        char *end = strchr(line, '\n');
        if (!end) end = line + strlen(line);
        char *p = line;
        while (p < end && (*p == ' ' || *p == '\t')) p++;
        if (p == line && (size_t)(end - p) >= 5 &&
            strncmp(p, "type ", 5) == 0) {
            char *eq = memchr(p + 5, '=', (size_t)(end - (p + 5)));
            if (eq) *eq = ' ';
        }
        line = *end ? end + 1 : end;
    }
...
static char *wisp_desugar_colon_definitions(const char *source) {
    SB out;
    sb_init(&out);
    ...
    while (*line) {
        ...
        if (p == line && identifier && colon && equal && colon < equal &&
            (equal + 1 >= end || equal[1] != '>') &&
            colon == after_name) {
            const char *type = colon + 1;
            while (type < equal && (*type == ' ' || *type == '\t')) type++;
            const char *type_end = equal;
            while (type_end > type &&
                   (type_end[-1] == ' ' || type_end[-1] == '\t')) type_end--;
            const char *value = equal + 1;
            while (value < end && (*value == ' ' || *value == '\t')) value++;
            sb_puts(&out, "(define [");
            for (const char *q = p; q < name_end; q++) sb_putc(&out, *q);
            sb_puts(&out, " : ");
            for (const char *q = type; q < type_end; q++) sb_putc(&out, *q);
            sb_puts(&out, "] ");
            for (const char *q = value; q < end; q++) sb_putc(&out, *q);
            sb_putc(&out, ')');
        } else {
            for (const char *q = line; q < end; q++) sb_putc(&out, *q);
        }
        if (*end == '\n') sb_putc(&out, '\n');
        line = *end ? end + 1 : end;
    }
...
static char *wisp_desugar_typed_lambda_literals(const char *source) {
    SB out;
    sb_init(&out);
    const char *p = source;
    while (*p) {
        const char *lambda = strstr(p, "λ");
        if (!lambda) {
            sb_puts(&out, p);
            break;
        }
        for (const char *q = p; q < lambda; q++) sb_putc(&out, *q);
        const char *name = lambda + strlen("λ");
        while (*name == ' ' || *name == '\t') name++;
        if (*name == '[') {
            const char *close = strchr(name + 1, ']');
            if (close && close[1] == '.') {
                sb_puts(&out, "λ");
                for (const char *q = name + 1; q < close; q++)
                    sb_putc(&out, *q);
                sb_putc(&out, '.');
                p = close + 2;
                continue;
        ...
        const char *newline = strchr(type, '\n');
        if (!dot || (newline && dot > newline)) {
            sb_puts(&out, "λ");
            p = lambda + strlen("λ");
            continue;
        }
        const char *type_end = dot;
        while (type_end > type &&
               (type_end[-1] == ' ' || type_end[-1] == '\t')) type_end--;
        const char *body = dot + 1;
        while (*body == ' ' || *body == '\t') body++;
        const char *body_end = body;
        while (*body_end && *body_end != '\n') body_end++;
        sb_puts(&out, "(lambda ([");
        for (const char *q = name; q < name_end; q++) sb_putc(&out, *q);
        sb_puts(&out, " : ");
        for (const char *q = type; q < type_end; q++) sb_putc(&out, *q);
        sb_puts(&out, "]) ");
        for (const char *q = body; q < body_end; q++) sb_putc(&out, *q);
        sb_putc(&out, ')');
        p = body_end;
    }
    return sb_take(&out);
...
/* Canonical diagram control flow.  The diagram is intentionally only a
 * surface spelling of Wisp's established receiver-relative guards:
 *
 *   n ├─ < 0 -> a              n | < 0 -> a
 *     ╰─╮                 =>     | = 0 -> b
 *       ├─ = 0 -> b                 | otherwise -> c
 *       ╰───▶ c
 *
 * Keeping the lowering here gives each drawing glyph exactly one role while
 * preserving the existing guard parser, type checker, and code generator.
 */
static const char *wisp_find_code_glyph(const char *line, const char *end,
                                        const char *glyph) {
    bool in_string = false;
    bool in_char = false;
    bool escape = false;
    ...
    for (const char *p = line; p < end; p++) {
        ...
        if ((in_string || in_char) && *p == '\\') { escape = true; continue; }
        if (!in_char && *p == '"') { in_string = !in_string; continue; }
        if (!in_string && *p == '\'') { in_char = !in_char; continue; }
        if (!in_string && !in_char && *p == ';') return NULL;
        if (!in_string && !in_char && p + glyph_len <= end &&
            memcmp(p, glyph, glyph_len) == 0)
            return p;
    }
    return NULL;
}

static char *desugar_glyph_control_flow(char *source) {
    ...
    while (*cursor) {
        ...
        const char *b = wisp_find_code_glyph(line, end, branch);
        const char *l = wisp_find_code_glyph(line, end, last);
        const char *t = wisp_find_code_glyph(line, end, turn);
        const char *v = wisp_find_code_glyph(line, end, leaf);

        if (b) {
            const char *rhs = b + strlen(branch);
            while (rhs < end && (*rhs == ' ' || *rhs == '\t')) rhs++;
            const char *arrow = strstr(rhs, "->");
            if (!arrow || arrow >= end)
                READER_ERROR(lineno, (int)(b - line) + 1,
                             "glyph control flow branch requires guarded leaf '->'");
            for (const char *p = line; p < b; p++) sb_putc(&out, *p);
            sb_puts(&out, "| ");
            for (const char *p = rhs; p < end; p++) sb_putc(&out, *p);
        } else if (l) {
            const char *p = l + strlen(last);
            while (p + strlen(dash) <= end &&
                   memcmp(p, dash, strlen(dash)) == 0)
                p += strlen(dash);
            if (p + strlen(turn) <= end &&
                memcmp(p, turn, strlen(turn)) == 0 &&
                p + strlen(turn) == end) {
                /* A descent connector is structural and occupies no syntax,
                 * but retain its newline for exact diagnostic line mapping. */
            } else if (p + strlen(leaf) <= end &&
                       memcmp(p, leaf, strlen(leaf)) == 0) {
                p += strlen(leaf);
                while (p < end && (*p == ' ' || *p == '\t')) p++;
                if (p == end)
                    READER_ERROR(lineno, (int)(l - line) + 1,
                                 "glyph control flow catch-all requires a leaf expression");
                for (const char *q = line; q < l; q++) sb_putc(&out, *q);
                sb_puts(&out, "| otherwise -> ");
                for (; p < end; p++) sb_putc(&out, *p);
            } else {
                READER_ERROR(lineno, (int)(l - line) + 1,
                             "glyph control flow final arm must descend with '╮' or terminate with '▶'");
            }
        } else if (t) {
            const char *p = t;
            while (p > line && p - line >= (ptrdiff_t)strlen(dash) &&
                   memcmp(p - strlen(dash), dash, strlen(dash)) == 0)
                p -= strlen(dash);
            const char *after = t + strlen(turn);
            while (after < end && (*after == ' ' || *after == '\t')) after++;
            if (p == t || after != end)
                READER_ERROR(lineno, (int)(t - line) + 1,
                             "glyph control flow opener must end with '─╮'");
            for (const char *q = line; q < p; q++) sb_putc(&out, *q);
        } else {
            if (v)
                READER_ERROR(lineno, (int)(v - line) + 1,
                             "glyph control flow uses '▶' only for an unguarded catch-all");
            for (const char *p = line; p < end; p++) sb_putc(&out, *p);
        }
        if (*cursor == '\n') {
            sb_putc(&out, '\n');
            cursor++;
...
static char *desugar_scrutinee_clauses(char *source) {
    ...
    while (*cursor) {
        ...
        bool pattern_brackets =
            memchr(content, '[', (size_t)(line_end - content)) != NULL;
        int top_level_parens = 0;
        int paren_depth = 0;
        for (const char *p = content; p < line_end; p++) {
            if (*p == '(') {
                if (paren_depth == 0) top_level_parens++;
                paren_depth++;
            } else if (*p == ')' && paren_depth > 0) {
                paren_depth--;
            }
        }
        bool tuple_pattern_clause = top_level_parens > 1;
        bool explicit_match =
            content < line_end && strncmp(content, "(match ", 7) == 0;
        bool candidate = content < line_end && *content != '|' &&
                         !existing_clause &&
                         !pattern_brackets &&
                         !tuple_pattern_clause &&
        ...
        if (explicit_match) {
            /* The programmer already supplied the match introducer.  This
             * pass only joins its indented clauses and supplies the closing
             * parenthesis; wrapping again produces `(match (match ... with
             * with ...)`. */
...
ASTList wisp_parse_all(const char *source, const char *filename) {
    WispRecoveryScope scope;
    memset(&scope, 0, sizeof(scope));
    scope.saved_active_arities = g_active_wisp_arities;
    scope.saved_param_kind = g_param_kind_is_func;
    scope.saved_known_function = g_is_known_function;
    scope.saved_filename = current_filename;
    scope.saved_source = current_source;
    scope.saved_original_source = original_source;
    scope.deferred = reader_diagnostic_cleanup_defer_current(
        &scope.cleanup, wisp_recovery_scope_release, &scope);
    scope.protected_scope = scope.deferred;
    wisp_pending_type_clear();

    /* Documentation is outside the language for every source consumer.
     * Strip it before scoped reader dispatch as well as before the ordinary
     * lexer; otherwise an English line beginning with a registered block name
     * (for example "configuration value") is misclassified as executable DSL
     * syntax. */
    char *reader_input = strdup(source ? source : "");
    scope.owned[0] = reader_input;
    if (!reader_input) {
        READER_ERROR(1, 1, "out of memory preparing commentary-free source");
    }
    wisp_normalize_newlines_in_place(reader_input);
    strip_commentary_sections_in_place(reader_input);
    reader_input = desugar_glyph_control_flow(reader_input);
    scope.owned[0] = reader_input;
    char *reader_expanded = reader_syntax_expand(reader_input, filename);
    free(reader_input);
    scope.owned[0] = reader_expanded;
    if (!reader_expanded) {
    ...
    scope.owned[0] = section_stripped;
    ...
    wisp_normalize_type_definition_equals(stripped);
    {
        char *typed_definitions = wisp_desugar_colon_definitions(stripped);
        free(stripped);
        stripped = typed_definitions;
        char *typed_lambdas = wisp_desugar_typed_lambda_literals(stripped);
        free(stripped);
        stripped = typed_lambdas;
        scope.owned[1] = stripped;
    }
    ...
    while (*p) {
        while (*p == ' ' || *p == '\t') p++;
        char c = *p;
        /* A collection literal may be the left operand of an unparenthesized
         * infix expression (`{1} ∪ {2}`, `[1] ++ [2]`).  Sending every line
         * that begins with `[` or `{` straight to the reader splits such a
         * line into three top-level forms before Wisp can promote the
         * operator. Standalone literals survive the Wisp path unchanged. */
        if (c && c != '\n' && c != ';' && c != '(' && c != '"')
            { has_wisp = true; break; }
        while (*p && *p != '\n') p++;
        if (*p) p++;
    ...
    if (!has_wisp) {
        ...
        scope.owned[1] = NULL;
        free(stripped);
        wisp_recovery_scope_finish(&scope);
        return result;
    }

    /* Build arity table */
    ArityTable *t = &scope.arities;
    if (g_ffi_arities_init)
        for (int i = 0; i < ARITY_BUCKETS; i++)
    ...
    arity_set(t, "match", -1);
    arity_set(t, "doc", -1);
    /* `define` owns a binder and one complete value/body form.  Treating it
     * as unary drops quoted values and detaches indented pattern clauses. */
    arity_set(t, "define", 2);
    arity_set(t, "var", 2);
    arity_set(t, "assert", 1);
    ...
    scope.stream = build_token_stream(stripped, t);
    WTokenStream *s = &scope.stream;

    /* Parse all top-level expressions */
    SB *out = &scope.builder;
    sb_init(out);
    scope.builder_active = true;
    bool first = true;
    while (s->pos < s->count) {
    ...
    scope.builder_active = false;
    scope.owned[2] = transformed;
    ...
    result = macro_expand_all(result.exprs, result.count);
    /* Colon sections are represented by Wisp with brackets while preserving
     * precedence.  Once the reader has produced a dedicated judgment node,
     * discard that transport-only wrapper on definition RHSs. */
    for (size_t i = 0; i < result.count; i++) {
        AST *form = result.exprs[i];
        if (form && form->type == AST_ARRAY &&
            form->array.element_count == 1 && form->array.elements[0] &&
            form->array.elements[0]->type == AST_JUDGMENT) {
            AST *judgment = form->array.elements[0];
            free(form->array.elements);
            form->array.elements = NULL;
            form->array.element_count = 0;
            form->array.element_capacity = 0;
            ast_free(form);
            result.exprs[i] = judgment;
            form = judgment;
        }
        if (!form || form->type != AST_LIST || form->list.count < 3 ||
            !form->list.items[0] || form->list.items[0]->type != AST_SYMBOL ||
            strcmp(form->list.items[0]->symbol, "define") != 0)
            continue;
        AST *rhs = form->list.items[2];
        if (rhs && rhs->type == AST_ARRAY && rhs->array.element_count == 1 &&
            rhs->array.elements[0] &&
            rhs->array.elements[0]->type == AST_JUDGMENT) {
            AST *judgment = rhs->array.elements[0];
            free(rhs->array.elements);
            rhs->array.elements = NULL;
            rhs->array.element_count = 0;
            rhs->array.element_capacity = 0;
            ast_free(rhs);
            form->list.items[2] = judgment;
        }
    }
