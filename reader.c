...
#define DA_PUSH(array, count, capacity, item) do {                 \
    if ((count) >= (capacity)) {                                   \
        ...
        (array) = realloc((array), sizeof(*(array)) * (capacity)); \
    }                                                              \
    (array)[(count)++] = (item);                                   \
} while(0)
...
static void compiler_error_range(int line, int column, int end_column, const char *fmt, ...) {
    ...
    if (src_to_print) {
        const char *line_start = src_to_print;
        int current_line = 1;
...
bool parse_int_type(const char *name, int line, int col,
    ...
    int width = atoi(digits);
    if (width < 1) {
        compiler_error(line, col,
            "invalid integer type '%s': bit-width must be at least 1", name);
    } else if (width >= 65 && width <= 127) {
        compiler_error(line, col,
            "invalid integer type '%s': bit-widths 65–127 are not supported; "
            "valid widths are 1–64 and 128", name);
    } else if (width > 128) {
        compiler_error(line, col,
            "invalid integer type '%s': bit-width %d exceeds the maximum of 128",
            name, width);
    }
    *out_width  = width;
    *out_signed = (prefix == 'I');
...
static bool is_symbol_char(unsigned char c) {
    return is_symbol_start(c) || (c>='0'&&c<='9') || c=='.' || c=='\'';
}
...
///// Character literal or quote
    ...
    // Postfix dot access: .fieldname (after a closing paren or bracket)
    if (c == '.') {
        char after = peek_ahead(lex, 1);
...
static void type_buf_append(char *buf, size_t size, const char *frag) {
...
static ASTParam parse_one_param(Parser *p) {
    ...
    if (p->current.type == TOK_SYMBOL) {
        ...
        if (is_colon || is_arrow) {
            ...
            if (is_arrow) {
                strncat(type_buf, "Pointer ::",
                        sizeof(type_buf) - strlen(type_buf) - 1);
            }

            int type_depth = 0;
            while (!(p->current.type == TOK_RBRACKET && type_depth == 0) &&
                ...
                if (tok_str) {
                    ...
                    if (cur_len > 0 && cur_len + 1 < sizeof(type_buf)) {
                        type_buf[cur_len++] = ' ';
                        type_buf[cur_len] = '\0';
                    }
                    if (cur_len + tok_len < sizeof(type_buf)) {
                        memcpy(type_buf + cur_len, tok_str, tok_len + 1);
                    }
                }
            ...
            if (type_buf[0]) {
                /* Only wrap in parens when the type is compound (contains
                 * spaces), e.g. "Pointer :: T" -> "(Pointer :: T)".
...
static void parse_fn_signature(Parser *p, ASTParam **out_params,
    ...
    g_typevar_counter = 0;
	    ...
	    if (is_arrow_sig) {
	        ...
	        while (peek_tok.type != TOK_EOF) {
	            ...
	            if (depth == 0 && peek_tok.type == TOK_SYMBOL &&
	                peek_tok.value && strcmp(peek_tok.value, "=>") == 0) {
	                has_constraint = true;
	                free(peek_tok.value);
	                break;
	            }
	        ...
	        if (has_constraint) {
	            while (p->current.type != TOK_EOF) {
	                bool is_constraint_arrow =
	                    p->current.type == TOK_SYMBOL &&
	                    p->current.value &&
	                    strcmp(p->current.value, "=>") == 0;
	                p->current = lexer_next_token(p->lexer);
	                if (is_constraint_arrow) break;
	            }
	        }
	    }
        ...
        if (p->current.type == TOK_SYMBOL &&
            ...
            if (p->current.type == TOK_LBRACKET) {
                ...
                bool rest_type_only = (p->current.value[0] >= 'A' && p->current.value[0] <= 'Z') ||
                ...
                /* Anonymous parenthesized typed rest: . (a -> Bool) */
                Lexer peek_lex = *p->lexer;
                Token peek_tok = lexer_next_token(&peek_lex);
                bool has_arrow_inside = false;
                int peek_depth = 0;
                while (peek_tok.type != TOK_RPAREN && peek_tok.type != TOK_EOF) {
                    if (peek_tok.type == TOK_LPAREN) peek_depth++;
                    if (peek_tok.type == TOK_RPAREN && peek_depth > 0) peek_depth--;
                    if (peek_tok.type == TOK_ARROW && peek_depth == 0) {
        ...
        if (p->current.type == TOK_LBRACKET) {
            ...
            /* Could be:
             *   (name :: Type)  — named typed param
             *   (a)             — anonymous collection param
             *   (Int -> Int)    — parenthesized arrow type, anonymous Fn param */

            /* Peek inside to detect arrow type: scan for '->' before ')' */
            Lexer peek_lex = *p->lexer;
            Token peek_tok = lexer_next_token(&peek_lex); /* first token inside '(' */
            bool has_arrow_inside = false;
            int peek_depth = 0;
            while (peek_tok.type != TOK_RPAREN && peek_tok.type != TOK_EOF) {
                if (peek_tok.type == TOK_LPAREN) peek_depth++;
            ...
            if (p->current.type == TOK_SYMBOL) {
                ...
                if (is_explicit_param) {
                    continue;
                }

                if (has_more_arrow) {
                    /* Not the final return type — treat as param.
                     * Lowercase = named param with inferred type (e.g. val).
                     * Uppercase = anonymous typed param (e.g. Int).        */
                    if (count >= capacity) {
                        capacity = capacity == 0 ? 4 : capacity * 2;
                        params   = realloc(params, sizeof(ASTParam) * capacity);
                    }
                    ...
                    /* Final return type. Consume one complete type expression.
                     * Bracketed collection types are valid inside return types,
                     * for example: Maybe [a]. */
                    char type_buf[512] = {0};
                    int paren_depth = 0;
                    int bracket_depth = 0;
                    while (p->current.type != TOK_EOF) {
                        ...
                        if (tok_str) {
                            bool no_space_before = (p->current.type == TOK_RBRACKET ||
                                                    p->current.type == TOK_RPAREN);
                            bool no_space_after_prev = false;
                            size_t cur_len = strlen(type_buf);
                            if (cur_len > 0) {
                                char prev = type_buf[cur_len - 1];
                                no_space_after_prev = (prev == '[' || prev == '(');
                            }
                            if (type_buf[0] && !no_space_before && !no_space_after_prev) {
                        ...
                        if (p->current.type == TOK_LPAREN) {
                            paren_depth++;
                        } else if (p->current.type == TOK_RPAREN && paren_depth > 0) {
                            paren_depth--;
                        } else if (p->current.type == TOK_LBRACKET) {
                            bracket_depth++;
                        } else if (p->current.type == TOK_RBRACKET && bracket_depth > 0) {
                            bracket_depth--;
                        }
                ...
                int peek_bracket_depth = 1;
                while (peek_tok.type != TOK_EOF && peek_bracket_depth > 0) {
                    if (peek_tok.type == TOK_LBRACKET)
                        peek_bracket_depth++;
                    else if (peek_tok.type == TOK_RBRACKET)
                        peek_bracket_depth--;
                    if (peek_bracket_depth == 0)
                        break;
                    free(peek_tok.value);
                    peek_tok = lexer_next_token(&peek_lex);
                }
                ...
                p->current = lexer_next_token(p->lexer); /* skip '[' */
                int bracket_depth = 0;
                while (!(p->current.type == TOK_RBRACKET && bracket_depth == 0) &&
                       p->current.type != TOK_EOF) {
                    if (p->current.type == TOK_LBRACKET)
                        bracket_depth++;
                    else if (p->current.type == TOK_RBRACKET && bracket_depth > 0)
    ...
    if (p->current.type == TOK_ARROW) {
        ...
        if (is_parenthesized) {
            ...
            char type_buf[512] = {0};
            int depth = 0;
            while (!(p->current.type == TOK_RPAREN && depth == 0) && p->current.type != TOK_EOF) {
                if (p->current.type == TOK_LPAREN) depth++;
                if (p->current.type == TOK_RPAREN) depth--;
                const char *tok_str = p->current.value ? p->current.value
                                    : (p->current.type == TOK_ARROW ? "->" : NULL);
...
static AST *parse_lambda(Parser *p) {
    ...
    // If the body starts with pmatch clauses (arrow at depth 0 before ')')
    ...
    if (p->current.type != TOK_RPAREN && p->current.type != TOK_EOF) {
        if (p->current.type == TOK_ARROW) {
            ...
            /* scan forward from current token */
            int depth = 0;
            Token scan = peek_cur;
            /* We need to scan starting from p->current without consuming it.
            ...
            while (!(scan_tok.type == TOK_RPAREN && depth == 0) &&
                ...
                if (scan_tok.type == TOK_LPAREN ||
                    scan_tok.type == TOK_LBRACKET) depth++;
                if ((scan_tok.type == TOK_RPAREN ||
                     scan_tok.type == TOK_RBRACKET) && depth > 0) depth--;
                if (scan_tok.type == TOK_ARROW && depth == 0) {
                    lambda_has_pmatch = true;
                    free(scan_tok.value);
                    break;
                }
            ...
            if (!lambda_has_pmatch) free(scan_tok.value);
        }
    }
    ...
    if (lambda_has_pmatch && count > 0) {
        ...
        for (int _i = 0; _i < pm->pmatch.clause_count; _i++) {
            ...
            for (int _j = 0; _j < cl->pattern_count; _j++)
                ast_pattern_free(&cl->patterns[_j]);
            free(cl->patterns);
        }
        free(pm->pmatch.clauses);
        free(pm);
...
static AST *parse_cond(Parser *p) {
    ...
    if (is_else) {
        ...
        while (p->current.type != TOK_RBRACKET && p->current.type != TOK_EOF) {
            DA_PUSH(body, body_count, body_cap, parse_expr(p));
        }
        if (p->current.type != TOK_RBRACKET)
            compiler_error(p->current.line, p->current.column,
...
static AST *parse_list(Parser *p) {
    ...
    // Detect (match target with clauses...)
    if (p->current.type == TOK_SYMBOL &&
        strcmp(p->current.value, "match") == 0) {
        p->current = lexer_next_token(p->lexer);

        AST *target = parse_expr(p);

        if (p->current.type != TOK_SYMBOL || strcmp(p->current.value, "with") != 0) {
            compiler_error(p->current.line, p->current.column,
        ...
        pm_params[0].type_name = NULL;
        pm_params[0].is_rest = false;
        pm_params[0].is_anon = true;

        AST *desugared = pmatch_desugar(pm, pm_params, 1);
        ...
        body_exprs[0] = desugared;
        AST *lam = ast_new_lambda(pm_params, 1, NULL, NULL, NULL, false, desugared, body_exprs, 1);

        AST *call = ast_new_list();
        ast_list_append(call, lam);
        ast_list_append(call, target);

        call->line = start_line;
        call->column = start_column;
        call->end_column = desugared->end_column;
    ...
    if (p->current.type == TOK_SYMBOL &&
        ...
        int   body_cap   = 0;
        while (p->current.type != TOK_RPAREN && p->current.type != TOK_EOF) {
            DA_PUSH(body_exprs, body_count, body_cap, parse_expr(p));
        }
...
///// Define
    ...
    if (p->current.type == TOK_SYMBOL &&
        ...
        if (p->current.type == TOK_LPAREN) {
            ...
            if (ret_type != NULL &&
                                        ...
                                        strcmp(p->current.value, "|") == 0));
                {
                    ...
                    while (!(peek_cur.type == TOK_RPAREN && depth == 0) &&
                         ...
                         * (#line N COL) — the lexer normally handles these but
                         * in peek mode we are using a cloned lexer that does NOT
                         * call lexer_next_token's #line handler, so the tokens
                         * leak through as LPAREN + SYMBOL("#line") + NUMBER + NUMBER + RPAREN.
                         * A stray LPAREN from a #line directive inflates depth and
                         * hides the '->' tokens we are looking for at depth 0.
                         * Detect and skip the entire (#line N COL) sequence here. */
                        if (peek_cur.type == TOK_LPAREN) {
                            Lexer tmp_lex = peek_lex;
                            Token tmp_tok = lexer_next_token(&tmp_lex);
                            bool is_line_directive = (tmp_tok.type == TOK_SYMBOL &&
                                                      tmp_tok.value &&
                                                      strcmp(tmp_tok.value, "#line") == 0);
                            free(tmp_tok.value);
                            if (is_line_directive) {
                                /* consume (#line N COL) entirely in peek_lex */
                                free(peek_cur.value);
                                peek_cur = lexer_next_token(&peek_lex); /* '#line' */
                                free(peek_cur.value);
                                peek_cur = lexer_next_token(&peek_lex); /* N */
                                free(peek_cur.value);
                                peek_cur = lexer_next_token(&peek_lex); /* COL */
                                free(peek_cur.value);
                                peek_cur = lexer_next_token(&peek_lex); /* ')' */
                                free(peek_cur.value);
                                peek_cur = lexer_next_token(&peek_lex); /* next real token */
                                continue;
                            }
                        }
                        if (peek_cur.type == TOK_LPAREN ||
                            peek_cur.type == TOK_LBRACKET) depth++;
                    ...
                    if (!body_has_arrow) free(peek_cur.value);
                }
                if (body_has_arrow) {
                    AST *pm = parse_pmatch_clauses(p, count);
                    pmatch_rename_anon_params(pm, params, count);
                    ...
                    free(pm);
                    body_cap = 1;
                    body_exprs = malloc(sizeof(AST*) * body_cap);
                    body_exprs[0] = desugared;
                    body_count = 1;
                } else {
                    while (p->current.type != TOK_RPAREN &&
            ...
            while (p->current.type != TOK_RPAREN &&
                   ...
                   p->current.type != TOK_KEYWORD) {
                AST *e = parse_expr(p);
                /* Skip #line directive nodes that leaked through as AST_LIST
                 * with head symbol "#line" — they are location markers only. */
                if (e->type == AST_LIST && e->list.count >= 1 &&
                    e->list.items[0]->type == AST_SYMBOL &&
                    strcmp(e->list.items[0]->symbol, "#line") == 0) {
                    ast_free(e);
                    continue;
                }
          ...
          define_value_path:;
            ...
            while (p->current.type == TOK_LPAREN) {
                /* peek: is it a #line node? */
                Lexer saved_lex = *p->lexer;
                Token saved_cur = p->current;
                Token peek = lexer_next_token(p->lexer);
                if (peek.type == TOK_SYMBOL && peek.value &&
                    strcmp(peek.value, "#line") == 0) {
                    /* consume the whole (#line N COL) */
                    free(peek.value);
                    while (p->current.type != TOK_RPAREN &&
                           p->current.type != TOK_EOF)
                        p->current = lexer_next_token(p->lexer);
    ...
    if (p->current.type == TOK_SYMBOL &&
        ...
        if (p->current.type == TOK_SYMBOL && p->current.value &&
            ...
            if (p->current.type != TOK_SYMBOL)
                compiler_error(p->current.line, p->current.column,
                               "Expected class name after superclass constraint");
            cls_name = my_strdup(p->current.value);
            p->current = lexer_next_token(p->lexer);
            if (p->current.type != TOK_SYMBOL)
                compiler_error(p->current.line, p->current.column,
                               "Expected type variable after class name");
            type_var = my_strdup(p->current.value);
            p->current = lexer_next_token(p->lexer);
        }
        ...
        while (p->current.type != TOK_RPAREN &&
            ...
            if (p->current.type == TOK_LBRACKET) {
                ...
                if (p->current.type == TOK_LPAREN) {
                    p->current = lexer_next_token(p->lexer); /* consume '(' */
                    if (p->current.type == TOK_SYMBOL) {
                        mname = my_strdup(p->current.value);
                        p->current = lexer_next_token(p->lexer);
                    }
                    if (p->current.type == TOK_RPAREN)
                        p->current = lexer_next_token(p->lexer);
                ...
                if (mname && has_type_sep) {
                    ...
                    while (p->current.type != TOK_EOF) {
                        ...
                        if (depth == 0 && p->current.type == TOK_LPAREN) {
                            ...
                            Token peek_tok = lexer_next_token(&peek_lex);
                            int peek_depth = 1;
                            while (peek_tok.type != TOK_EOF && peek_depth > 0) {
                                if (peek_tok.type == TOK_LPAREN) peek_depth++;
                                if (peek_tok.type == TOK_RPAREN) peek_depth--;
                                free(peek_tok.value);
                                if (peek_depth > 0) peek_tok = lexer_next_token(&peek_lex);
                            }
                            peek_tok = lexer_next_token(&peek_lex);
                            bool is_next_decl =
                                (peek_tok.type == TOK_SYMBOL && peek_tok.value &&
                        ...
                        if (depth == 0 && p->current.type == TOK_SYMBOL &&
                            p->current.value && strcmp(p->current.value, "class") == 0) {
                            break;
                        }
                    ...
                    register_local_func_arity(mname, 1);
                    method_count++;
                    continue;
                }
            ...
            if (p->current.type == TOK_LPAREN) {
                ...
                if (is_sig && peek_name) {
                    ...
                    int depth = 0;
                    while (p->current.type != TOK_EOF) {
                        if (depth == 0 && p->current.type == TOK_RPAREN) break;
                        ...
                        if (depth == 0 && p->current.type == TOK_LPAREN) {
                            ...
                            peek_tok = lexer_next_token(&peek_lex);
                            bool is_next_decl = (peek_tok.type == TOK_SYMBOL && peek_tok.value &&
                                                (strcmp(peek_tok.value, "::") == 0 || strcmp(peek_tok.value, "=>") == 0));
                            free(peek_tok.value);
                            if (is_next_decl) break;
                        }
    ...
    if (p->current.type == TOK_SYMBOL &&
        ...
        while (p->current.type != TOK_RPAREN &&
            ...
            if (p->current.type == TOK_SYMBOL && p->current.value) {
                 ...
                 * If pat1.var_name matches a known method and p->current is a param,
                 * it is prefix: (method param1 param2).
                 * If p->current.value matches a known method, it is infix:
                ...
                /* Check if pat1's name is a known method */
                bool pat1_is_method = false;
                if (pat1.kind == PAT_VAR && pat1.var_name) {
                    for (int _mi = 0; _mi < nethods; _mi++) {
                        if (strcmp(methods[_mi].method, pat1.var_name) == 0) {
                            pat1_is_method = true;
                            break;
                        }
                    }
                ...
                /* Determine if the CURRENT token (after pat1) looks like an operator.
                 * Operators contain non-alpha, non-underscore, non-digit characters.
                 * If the token after pat1 is an operator, we are in infix position:
                 *   (ma >>= f)  — ma is a value pattern, >>= is the infix method
                 * If the token after pat1 is a lowercase word or =>, we are prefix:
                 *   (return x)  — return is the method name, x is the pattern arg
                 *   (>>= ma f)  — >>= is the method (pat1_is_op catches this) */
                bool next_is_op_symbol = false;
                if (p->current.type == TOK_SYMBOL && p->current.value) {
                    const char *sv = p->current.value;
                    bool all_word = true;
                    for (int _ci = 0; sv[_ci]; _ci++) {
                        char _c = sv[_ci];
                    ...
                    next_is_op_symbol = !all_word;
                }

                bool is_prefix = pat1_is_op
                ...
                /* If current token is explicitly a known method name, force infix */
                if (current_is_method && !pat1_is_op && pat1.kind != PAT_CONSTRUCTOR) {
                    is_prefix = false;
                }

                if (is_prefix) {
                    /* prefix: (method pat1 pat2) — pat1 was the method name */
    ...
    if (p->current.type == TOK_SYMBOL &&
        ...
        while (p->current.type == TOK_SYMBOL && p->current.value &&
            ...
            type_params[type_param_count++] = my_strdup(p->current.value);
            p->current = lexer_next_token(p->lexer);
        }

        /* Consume '=' separator if present: data Maybe a = Nothing | Just a */
        if (p->current.type == TOK_SYMBOL && p->current.value &&
            strcmp(p->current.value, "=") == 0) {
            p->current = lexer_next_token(p->lexer);
        }
        ...
        while (p->current.type != TOK_RPAREN &&
            ...
            /* Supports both bare types (Just Float) and bracket syntax (Just [value :: Float]) */
            while (!(p->current.type == TOK_SYMBOL &&
                   ...
                   !(p->current.type == TOK_SYMBOL &&
                     ...
                     strcmp(p->current.value, "deriving") == 0)) {
                if (p->current.type == TOK_SYMBOL && p->current.value) {
                    /* Bare type name, including type parameters: Just a, Left a, Right b */
                    /* If it matches a known type parameter, record it as-is */
                    ctor.field_types = realloc(ctor.field_types,
                        sizeof(char*) * (ctor.field_count + 1));
                    ctor.field_types[ctor.field_count++] =
                        my_strdup(p->current.value);
                    ...
                    while ((p->current.type != TOK_RBRACKET || depth > 0) &&
                        ...
                        if (p->current.type == TOK_RBRACKET && depth > 0) depth--;
                        if (p->current.type == TOK_RBRACKET && depth == 0) break;
                        if (p->current.value) {
                            if (type_buf[0]) strncat(type_buf, " ",
                                sizeof(type_buf) - strlen(type_buf) - 1);
                    ...
                    if (type_buf[0]) {
                        ctor.field_types = realloc(ctor.field_types,
                            sizeof(char*) * (ctor.field_count + 1));
                        ctor.field_types[ctor.field_count++] = my_strdup(type_buf);
                    }
                } else if (p->current.type == TOK_LPAREN) {
                    /* Parenthesized type: (Json) or (String Json) or ((String Json))
                     * Collect the entire (...) including nested parens as a type string */
                    char type_buf[512] = {0};
                    int depth = 0;
                    while (p->current.type != TOK_EOF) {
                        if (p->current.type == TOK_LPAREN) {
                            ...
                            if (type_buf[0] && type_buf[strlen(type_buf)-1] != '(') {
                                strncat(type_buf, " ",
                                    sizeof(type_buf) - strlen(type_buf) - 1);
                            }
                            if (p->current.value) {
    ...
    if (p->current.type == TOK_SYMBOL &&
        ...
        /* Build params with types from sig parts. Method clauses are parsed
         * as pmatch clauses, so the lambda argument names must be synthetic;
         * clause names like c are pattern binders, not lambda parameters. */
        ASTParam *mparams = malloc(sizeof(ASTParam) * (mparam_count ? mparam_count : 1));
        for (int i = 0; i < mparam_count; i++) {
            char pname_buf[64];
            snprintf(pname_buf, sizeof(pname_buf), "__method_arg_%d", i);
            mparams[i].name = my_strdup(pname_buf);
        ...
        if (mparam_count > 0 &&
            ...
            int scan_depth = 0;
            while (!(scan_tok.type == TOK_RPAREN && scan_depth == 0) &&
                   scan_tok.type != TOK_EOF &&
                   scan_tok.type != TOK_KEYWORD) {
                if (scan_tok.type == TOK_LPAREN ||
                    scan_tok.type == TOK_LBRACKET ||
                    scan_tok.type == TOK_LBRACE ||
                    scan_tok.type == TOK_HASH_LBRACE) {
                    scan_depth++;
                }

                if ((scan_tok.type == TOK_RPAREN ||
                     scan_tok.type == TOK_RBRACKET ||
                     scan_tok.type == TOK_RBRACE) &&
                    scan_depth > 0) {
                    scan_depth--;
                }

                if (scan_depth == 0 &&
                    (scan_tok.type == TOK_ARROW ||
                     scan_tok.type == TOK_PIPE ||
                     (scan_tok.type == TOK_SYMBOL &&
                      scan_tok.value &&
                      strcmp(scan_tok.value, "|") == 0))) {
                    method_has_pmatch = true;
                    free(scan_tok.value);
                    break;
                }
        ...
        if (p->current.type != TOK_RPAREN) {
            compiler_error(p->current.line, p->current.column,
                           "Expected ')' to close method definition");
        }
        p->current = lexer_next_token(p->lexer);

        AST **body_exprs2 = malloc(sizeof(AST*));
        body_exprs2[0] = body_expr;
        AST *method_lam = ast_new_lambda(mparams, mparam_count,
     ...
     *   (module Name [a b c])
     *   (module Data.Char [a b c])
     *
     * New grouped style:
     *   (module Data.Char where
     *     :ascii nul tab lf
     *     :predicates lower? upper?)
     *
     * Group keywords are metadata for later. For now we flatten the exported
     * names into the old export array so the existing backend keeps working.
     */
    if (p->current.type == TOK_SYMBOL &&
        strcmp(p->current.value, "module") == 0) {
        int mod_line = p->current.line;
        int mod_col  = p->current.column;

        p->current = lexer_next_token(p->lexer); /* consume module */
        ...
        strncat(module_name, p->current.value,
                sizeof(module_name) - strlen(module_name) - 1);
        p->current = lexer_next_token(p->lexer);
        ...
        AST *exports = ast_new_array();

        if (p->current.type == TOK_SYMBOL &&
            p->current.value &&
            strcmp(p->current.value, "where") == 0) {
            p->current = lexer_next_token(p->lexer); /* consume optional where */
        }
        ...
        if (p->current.type == TOK_LBRACKET) {
            ...
            while (p->current.type != TOK_RBRACKET &&
                   p->current.type != TOK_EOF) {
                if (p->current.type == TOK_SYMBOL && p->current.value) {
                    ast_array_append(exports, ast_new_symbol(p->current.value));
                    p->current = lexer_next_token(p->lexer);
                    continue;
                }

                if (p->current.type == TOK_DOT && p->current.value) {
                    ast_array_append(exports, ast_new_symbol(p->current.value));
                    p->current = lexer_next_token(p->lexer);
                    continue;
                }
                ...
                ast_array_append(exports, item);
            }

            if (p->current.type != TOK_RBRACKET) {
                compiler_error(p->current.line, p->current.column,
                               "Expected ']' to close module export list");
            }
            ...
            while (p->current.type != TOK_RPAREN &&
                   p->current.type != TOK_EOF) {
                if (p->current.type == TOK_KEYWORD) {
                    /* Group metadata like :ascii, :predicates, :case.
                     * For now ignore the group key and keep the names. */
                    p->current = lexer_next_token(p->lexer);
                    continue;
                }
        ...
        if (p->current.type != TOK_RPAREN) {
            compiler_error(p->current.line, p->current.column,
                           "Expected ')' to close module declaration");
        }

        p->current = lexer_next_token(p->lexer); /* consume ')' */

        AST *result = ast_new_list();
        ast_list_append(result, ast_new_symbol("module"));
        ast_list_append(result, ast_new_symbol(module_name));
        ast_list_append(result, exports);

        ast_free(list);

        result->line   = mod_line;
        result->column = mod_col;
        return result;
    }
    ...
    if (p->current.type == TOK_SYMBOL &&
        ...
        /* Optional progress cookie:
         *   tests [1/5]
         *   tests [/]
         * For now this is metadata only. Parse it and ignore it.
         * Later the test runner can update it automatically. */
        if (p->current.type == TOK_LBRACKET) {
            int depth = 1;
            p->current = lexer_next_token(p->lexer);
            ...
            if (depth != 0) {
                compiler_error(p->current.line, p->current.column,
                               "Expected ']' to close tests progress cookie");
            }
        }
...
static AST *parse_bracket_list(Parser *p) {
    ...
    ssize_t pipe_index = -1;
    for (size_t i = 0; i < list->list.count; i++) {
        AST *item = list->list.items[i];
        if (item->type == AST_SYMBOL && item->symbol &&
            strcmp(item->symbol, "|") == 0) {
            pipe_index = (ssize_t)i;
            break;
        }
    }
    if (pipe_index >= 0) {
        if (pipe_index == 0 || (size_t)pipe_index + 1 >= list->list.count) {
            compiler_error(start_line, start_column,
                           "[E-READER-LIST-CONS] list-cons syntax must be [head | tail]");
        }

        AST *result = NULL;
        if ((size_t)pipe_index + 2 == list->list.count) {
            result = list->list.items[pipe_index + 1];
        } else {
            result = ast_new_list();
            for (size_t i = (size_t)pipe_index + 1; i < list->list.count; i++)
                ast_list_append(result, list->list.items[i]);
            result->line = start_line;
            result->column = start_column;
            result->end_column = end_column;
        }
        for (ssize_t i = pipe_index - 1; i >= 0; i--) {
            AST *node = ast_new_list();
            ast_list_append(node, ast_new_symbol("rt_coll_concat"));
            ast_list_append(node, list->list.items[i]);
            ast_list_append(node, result);
            node->line = start_line;
            node->column = start_column;
...
static AST *parse_expr_base(Parser *p) {
    Token tok = p->current;
    ...
    case TOK_NUMBER: {
        ...
        // Regular number
        AST *ast = ast_new_number(0.0, tok.value);
        if (tok.value) {
            if (tok.value[0] == '0' && (tok.value[1] == 'x' || tok.value[1] == 'X')) {
                ast->raw_int = strtoull(tok.value, NULL, 16);
                ast->has_raw_int = true;
                ast->number = (double)(uint64_t)ast->raw_int;
            } else if (tok.value[0] == '0' && (tok.value[1] == 'b' || tok.value[1] == 'B')) {
                ast->raw_int = strtoull(tok.value + 2, NULL, 2);
                ast->has_raw_int = true;
                ast->number = (double)(uint64_t)ast->raw_int;
            } else if (tok.value[0] == '0' && (tok.value[1] == 'o' || tok.value[1] == 'O')) {
                ast->raw_int = strtoull(tok.value + 2, NULL, 8);
                ast->has_raw_int = true;
                ast->number = (double)(uint64_t)ast->raw_int;
            } else {
...
///// Function-call sugars
    ...
    while (p->current.type == TOK_DOT && p->current.value) {
        const char *field = p->current.value;
        if (!field[0]) break;
        int dot_line = p->current.line;
        int dot_col  = p->current.column;
        p->current = lexer_next_token(p->lexer);
        ...
        if (expr_result->type == AST_SYMBOL) {
            ...
            AST *merged = ast_new_symbol(combined);
            merged->line       = expr_result->line;
            merged->column     = expr_result->column;
            merged->end_column = dot_col + (int)strlen(field) + 1;
    ...
    /* Assignment sugar: expr <- rhs */
    if (p->current.type == TOK_SYMBOL && p->current.value &&
        strcmp(p->current.value, "<-") == 0) {

        int arrow_line = p->current.line;
        int arrow_col  = p->current.column;
        p->current = lexer_next_token(p->lexer); // consume '<-'

        AST *rhs = parse_expr(p);

        AST *set_node = ast_new_list();
        AST *set_sym = ast_new_symbol("set!");
        set_sym->literal_str = my_strdup("<-");

        ast_list_append(set_node, set_sym);
        ast_list_append(set_node, expr_result);
        ast_list_append(set_node, rhs);

        set_node->line       = arrow_line;
        set_node->column     = arrow_col;
        set_node->end_column = rhs->end_column;

        expr_result = set_node;
    }
